...
/**
 * Testcase DDMMultipleFormat. This test class verifies using multiple format
 * logical files: <ul compact>
 * <li>reading
 * <li>writing
 * </ul>
 **/
...
//  AS400 pwrSys_ = DDMTest.PwrSys;
   ...
   **/
  /*
   * public DDMMultipleFormat(AS400 systemObject, Vector<String> variationsToRun,
   * int runMode, FileOutputStream fileOutputStream, ) { // The third parameter is
   * the total number of variations in this class. super(systemObject,
   * "DDMMultipleFormat", 1, variationsToRun, runMode, fileOutputStream); pwrSys_
   * = pwrSys; }
   */
   ...
   * Constructor. This is called from the DDMTest constructor.
   **/
  public DDMMultipleFormat(AS400 systemObject, Vector<String> variationsToRun, int runMode,
      FileOutputStream fileOutputStream,

      String testLib, AS400 pwrSys) {
    // The third parameter is the total number of variations in this class.
    super(systemObject, "DDMMultipleFormat", 1, variationsToRun, runMode, fileOutputStream);
    pwrSys_ = pwrSys;
//    if (testLib != null)
//    {
    setTestLib(testLib);
//    }
  ...
  protected void setup() throws Exception {
    try {
      ...
      } // may not exist, so ignore faliures
      ...
      AS400FileRecordDescription rd = new AS400FileRecordDescription(systemObject_,
          "/qsys.lib/ddmtestsav.lib/simpleseq.file");
      ...
      SequentialFile f1 = new SequentialFile(systemObject_, "/QSYS.LIB/DDMTESTSAV.LIB/DDMLOCK.FILE/MBR1.MBR");
      try {
  ...
  /**
   * Change the default wait time so that the tests run faster. This did not work
   * as the open does not respect the default wait time on the open
   * 
   * @param as400
   ...
   */
  private void changeWaitTime(AS400 as400) throws AS400SecurityException, IOException, ErrorCompletingRequestException,
      InterruptedException, ObjectDoesNotExistException {

    as400.connectService(AS400.RECORDACCESS);

    Job[] jobs = as400.getJobs(AS400.RECORDACCESS);

    for (int i = 0; i < jobs.length; i++) {
      Job job = jobs[i];
      job.setDefaultWait(5);
      job.commitChanges();
    }
  ...
  /**
   * Verify multpiple format logical file.. <ul compact>
   * <li>Read from the file specifying the different formats associated with the
   * file.
   * </ul>
   * Expected results: <ul compact>
  ...
  /**
   * Verify AS400File.lock(int) with a lock type of READ_EXCLUSIVE_LOCK. <ul
   * compact>
   * <li>Set the lock when the file is closed.
   * </ul>
   * Expected results: <ul compact>
   * <li>Another connection should not be able to open the file for read,
   * read/write or write.
   * </ul>
   **/
  public void Var002() {
    ...
    AS400 violatorAS400 = null;

    try {

      file = new SequentialFile(systemObject_, "/QSYS.LIB/DDMTESTSAV.LIB/DDMLOCK.FILE/MBR1.MBR");
      file.lock(AS400File.READ_EXCLUSIVE_LOCK);

      violatorAS400 = new AS400(systemObject_);
      changeWaitTime(violatorAS400);
      violator = new SequentialFile(violatorAS400, file.getPath());
  ...
  public void Var004() {
    ...
    SequentialFile violator = null;
    AS400 violatorAS400 = null;
    AS400 file2AS400 = null; 

    try {
      file1 = new SequentialFile(systemObject_, "/QSYS.LIB/DDMTESTSAV.LIB/DDMLOCK.FILE/MBR1.MBR");
      file1.lock(AS400File.READ_ALLOW_SHARED_READ_LOCK);
      file2AS400 = new AS400(systemObject_);
      file2 = new SequentialFile(file2AS400, file1.getPath());
      ...
      violator = new SequentialFile(violatorAS400, file1.getPath());
    ...
    if (violatorAS400 != null) violatorAS400.close(); 
  ...
  public void Var005() {
    ...
    if (file2AS400 != null) file2AS400.close(); 
    if (violatorAS400 != null) violatorAS400.close(); 
   ...
   * </ul>
   * Expected results: <ul compact>
   * <li>Another connection should be able to open the file for read, read/write
   * or write.
  ...
  /**
   * Verify AS400File.lock(int) with a lock type of WRITE_EXCLUSIVE_LOCK. <ul
   * compact>
   * <li>Set the lock when the file is closed.
   * </ul>
   * Expected results: <ul compact>
  ...
  /**
   * Verify AS400File.getExplicitLocks(). <ul compact>
   ...
   * <li>One lock set will return an int[] of size one containing the lock that
   * was set.
   * <li>All locks set will return an int[] of size six containing the locks that
   * were set.
   * </ul>
   **/
  public void Var014() {
    ...
    try {
      ...
      if (file.getExplicitLocks().length == 0) {
        ...
        if (locks.length == 1 && locks[0] == AS400File.READ_EXCLUSIVE_LOCK) {
          ...
          assertCondition(locks.length == 6 && v.contains(Integer.valueOf(AS400File.READ_EXCLUSIVE_LOCK))
  ...
  /**
   * Verify AS400File.releaseExplicitLocks(). <ul compact>
  ...
  /**
   * Verify record locks when file opened for read_only with commitment control
   * locking of *CS.<br>
   ...
   * Expected results:
   * <ul>
   * <li>A record lock of *READ will be held on a read record until the cursor is
   * moved from that record.
   * </ul>
   **/
  public void Var017() {
    if (true) { 
      notApplicable("Testcase needs work");
    } else { 
    setVariation(17);
    SequentialFile file = null;
    try {
      file = new SequentialFile(systemObject_, fileName_);
      DDMLockFormat lockFormat = new DDMLockFormat(systemObject_);
      lockFormat.setName("FMT1");
      file.setRecordFormat(lockFormat);
      // Populate the file
      ...
      file.readFirst();
      output_.println(
          "Does a *READ record lock exist on record number 1 of file DDMLOCK in " + "DDMTest" + "(Y/N)? (DSPRCDLCK)");
      ...
      file.readNext();
      output_.println(
          "Does a *READ record lock exist on record number 2 of file DDMLOCK in " + "DDMTest" + "(Y/N)? (DSPRCDLCK)");
      resp = inBuf.readLine();
      if (!resp.equals("y") && !resp.equals("Y")) {
        failed("Verification of record lock after readFirst failed");
        file.commit();
