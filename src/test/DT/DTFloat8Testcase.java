...
public class DTFloat8Testcase extends Testcase
{
    ...
    public void Var005()
    {
        ...
        {
            AS400Float8 conv = new AS400Float8();
            byte[] data = conv.toBytes(new Double(0.5546875));
            if (data.length == 8 &&
                data[0] == (byte)0x3F &&
                data[1] == (byte)0xE1 &&
                data[2] == (byte)0xC0 &&
    ...
    public void Var009()
    {
        try
        {
            ...
            for (int i=0; i < 6; ++i)
            {
                int ret = conv.toBytes(new Double(testValue[i]), data);
    ...
    public void Var017()
    {
        try
        {
            ...
            int ret = conv.toBytes(new Double(0.0), data, 0);
            if (ret != 8)
            {
                valid = false;
            }
            ...
            ret = conv.toBytes(new Double(0.0), data, 5);
            if (ret != 8)
            {
                valid = false;
            }
            ...
            ret = conv.toBytes(new Double(0.0), data, 16);
            if (ret != 8)
            {
                valid = false;
            }
