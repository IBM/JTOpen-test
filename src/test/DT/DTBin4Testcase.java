...
public class DTBin4Testcase extends Testcase
{
  public static void main(String args[]) throws Exception {
    ...
    public void Var005()
    {
        ...
        {
            AS400Bin4 conv = new AS400Bin4();
            byte[] data = conv.toBytes(Integer.valueOf(-22216467));
            if (data.length == 4 &&
                data[0] == (byte)0xFE &&
                data[1] == (byte)0xAD &&
                data[2] == (byte)0x00 &&
            ...
            for (int w = 0x00; w <= 0xFF; w+=0x80)
            {
                for (int x = 0x00; x <= 0xFF; x+=0x80)
                {
                    for (int y = 0x00; y <= 0xFF; y+=0x80)
                    {
                        ...
                        {
                            int ret = conv.toBytes(Integer.valueOf(testValue[i++]), data);
    ...
    public void Var017()
    {
        try
        {
            ...
            ret = conv.toBytes(Integer.valueOf(0), data, 4);
            if (ret != 4)
            {
                valid = false;
            }
    ...
    public void Var018()
    {
        ...
        try
        {
            int ret = conv.toBytes(Integer.valueOf(0), new byte[3], 0);
            failed("Did not throw exception. ret="+ret);
        }
        catch (Exception e)
        {
    ...
    public void Var019()
    {
        ...
        try
        {
            int ret = conv.toBytes(Integer.valueOf(0), new byte[7], 5);
            failed("Did not throw exception. ret="+ret);
        }
        catch (Exception e)
        {
    ...
    public void Var020()
    {
        ...
        try
        {
            int ret = conv.toBytes(Integer.valueOf(0), new byte[10], -5);
            failed("Did not throw exception. ret="+ret);
        }
        catch (Exception e)
        {
