...
/**
 * Testcase JDCSReturnValue. This tests the use of return value parameters in
 * the JDBC CallableStatement class.
 **/
public class JDCSReturnValue extends JDTestcase {
  public static void main(String args[]) throws Exception {
    String[] newArgs = new String[args.length + 2];
    newArgs[0] = "-tc";
    newArgs[1] = "JDCSReturnValue";
   ...
   * Constructor.
   **/
  public JDCSReturnValue(AS400 systemObject, Hashtable<String, Vector<String>> namesAndVars, int runMode,
      FileOutputStream fileOutputStream,

      String password) {
    super(systemObject, "JDCSReturnValue", namesAndVars, runMode, fileOutputStream, password);
  }
    ...
    if (checkReturnValueSupport()) {
      try {
        CallableStatement cs = connection_
            .prepareCall("   ?      =     CALL      " + JDSetupProcedure.STP_CSPARMSRV + "(?,?,?)");
        cs.setInt(2, 0);
        cs.setInt(4, 0);
        cs.registerOutParameter(1, Types.INTEGER);
        cs.registerOutParameter(3, Types.INTEGER);
  ...
  /**
   * Call a stroed procedure that has a return value, but do no register or get
   * the return value.
   **/
  public void Var006() {
    if (checkReturnValueSupport()) {
  ...
  /**
   * Call a stored procedure that has a return value, but do no deal with the
   * return value at all.
   **/
  public void Var007() {
    if (checkReturnValueSupport()) {
  ...
  /**
   * Call a stored procedure that does not have any parameters, but returns a
   * value.
   **/
  public void Var009() {
    if (checkReturnValueSupport()) {
  ...
  public void Var010() {
    ...
    if (checkReturnValueSupport()) {
      ...
      try {
        ...
        if (!success) {
          sb.append("\ncs.getInt(1)=" + cs.getInt(1) + " sb 1334 " + "cs.getInt(3)=" + cs.getInt(3) + " sb 1333 "
