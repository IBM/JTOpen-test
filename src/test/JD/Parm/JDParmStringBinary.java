...
**/
    ...
    throws Exception
    {
	   ...
	   connection = testDriver_.getConnection(baseURL_, userId_, encryptedPassword_);
           ...
           stringsTable_ = JDParmTest.COLLECTION+".strbin ";

           
           // Create a table that uses the largest row size the database will
           // allow me to use.
           s.executeUpdate("create or replace table "+ stringsTable_ + 
                           "(col1 char(1) CCSID 65535, " +
                           "col2 char(20) CCSID 65535, " + 
                           "col3 char (32744) CCSID 65535) on replace delete rows");
   ...
   public void Var002() {
      try {
         ...
         if (getDriver () == JDTestDriver.DRIVER_TOOLBOX) 
             expected = "\u0030\u0030";
         if (count == 1)
            assertCondition(JDParmHelper.verifyString("col1", expected, connection,stringsTable_,output_));
         else
            failed("invalid update count");
   ...
   public void Var028() {
      try {
         ...
         if (count == 1) {
             ...
             assertCondition(JDParmHelper.verifyString("col3", expected.toString(), connection2,stringsTable_,output_));
         }
         else
            failed("invalid update count");
