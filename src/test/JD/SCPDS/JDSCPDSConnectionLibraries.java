...
//
// Filename:  JDSCPDSConnectionLibraries.java
//
...
/**
Testcase JDSCPDSConnectionLibraries.  This tests the following
...
Constructor.
**/
    public JDSCPDSConnectionLibraries (AS400 systemObject,
                                      ...
                                      String password)
    {
        super (systemObject, "JDSCPDSConnectionLibraries",
...
**/
    ...
    throws Exception
    {
        if (isJdbc20StdExt()) {
            ...
            JDReflectionUtil.callMethod_V("setPassword",PasswordVault.decryptPasswordLeak(encryptedPassword_));
            //JDReflectionUtil.callMethod_V("setDecimalSeparator("bogus");
    ...
    public void Var001()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                JDReflectionUtil.callMethod_V(dataSource,"setUser",userId_);
                JDReflectionUtil.callMethod_V("setPassword",PasswordVault.decryptPasswordLeak(encryptedPassword_));
                JDReflectionUtil.callMethod_V("setNamingOption","sql");
                Connection c = dataSource.getConnection ();
    ...
    public void Var002()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                JDReflectionUtil.callMethod_V(dataSource,"setUser",userId_);
                JDReflectionUtil.callMethod_V("setPassword",PasswordVault.decryptPasswordLeak(encryptedPassword_));
                JDReflectionUtil.callMethod_V("setNamingOption","system");
                Connection c = dataSource.getConnection ();
                String defaultSchema = getDefaultSchema (c);
                c.close ();
    ...
    public void Var003()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                c.close ();
                assertCondition (success, userId_+" not found in library list -- testcase updated 04/26/2006");
            }
            catch (Exception e) {
                failed(e, "Unexpected exception");
            }
    ...
    public void Var004()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                JDReflectionUtil.callMethod_V(dataSource,"setUser",userId_);
                JDReflectionUtil.callMethod_V("setPassword",PasswordVault.decryptPasswordLeak(encryptedPassword_));
                JDReflectionUtil.callMethod_V("setLibraries",JDSCPDSTest.COLLECTION
                                        + " " + COLLECTION2);
                Connection c = dataSource.getConnection ();
                boolean successA = checkLibraryList (c, JDSCPDSTest.COLLECTION);
                boolean successB = checkLibraryList (c, COLLECTION2);
                c.close ();
                assertCondition (successA && successB, 
                      JDSCPDSTest.COLLECTION+ " found in library list = "+successA+" "+
                      COLLECTION2 + " found int library list = "+successB+" testcase fixed 04/26/2006"
                    );
            }
            catch (Exception e) {
                failed(e,"Unexpected Exception");
        ...
        if (checkJdbc20StdExt()) {
            try {
               ...
               JDReflectionUtil.callMethod_V(dataSource,"setDatabaseName",system_);
                ...
                boolean successB = checkLibraryList (c, COLLECTION2);
                c.close ();
                assertCondition (successA && successB, JDSCPDSTest.COLLECTION+ " found in library list = "+successA+" "+
                    COLLECTION2 + " found int library list = "+successB+" testcase fixed 04/26/2006");
            }
            catch (Exception e) {
                failed(e,"Unexpected Exception");
    ...
    public void Var006()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                JDReflectionUtil.callMethod_V(dataSource,"setUser",userId_);
                JDReflectionUtil.callMethod_V("setPassword",PasswordVault.decryptPasswordLeak(encryptedPassword_));
                JDReflectionUtil.callMethod_V("setLibraries","BADLIBRARY");
                Connection c = dataSource.getConnection ();
                if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
