...
/**
 * Testcase JDLobBlobLocator. This tests the following method of the JDBC Blob
 * class:
 * 
 * <ul>
 * <li>getBinaryStream()
 ...
 * </ul>
 **/
public class JDLobBlobLocator extends JDTestcase {
   ...
   * Constructor.
   **/
  public JDLobBlobLocator(AS400 systemObject, Hashtable<String, Vector<String>> namesAndVars, int runMode,
      FileOutputStream fileOutputStream, String password) {
    super(systemObject, "JDLobBlobLocator", namesAndVars, runMode, fileOutputStream, password);
  }
  ...
  protected void setup() throws Exception {
    ...
    if (isJdbc20()) {
      if (areLobsSupported()) {
        ...
        statement_.executeUpdate("CREATE TABLE " + TABLE_ + "(C_BLOB BLOB(" + WIDTH_ + "), ID INT )");

        statement_.executeUpdate("CREATE TABLE " + TABLE2_ + "(C_BLOB BLOB(204800))");
        ...
        PreparedStatement ps = connection_.prepareStatement("INSERT INTO " + TABLE_ + " (C_BLOB, ID) VALUES (?,?)");
        ...
        String cProgram[] = { "   /* File bloblocinf.c created on Thu Feb 22 2007. */", "", "  /*",
            "  * UDF to get info about a blob from a locator", "", "", "    compile using the following ",
            "   CRTCMOD MODULE(BLOBLOCINF) DBGVIEW(*ALL)   ", "  CRTSRVPGM BLOBLOCINF export(*all)", "",
            "   CREATE FUNCTION BLOBLOCINF (int) RETURNS VARCHAR(200) ", "   LANGUAGE C EXTERNAL NAME ",
            "   'QGPL/BLOBLOCINF(BLOBLOCINF)', PARAMETER STYLE SQL", "", "   To test", "",
            "    select BLOBLOCINF(256) from qsys2.qsqptabl", "", "", "", "", "  */", "", "#include <stdlib.h>",
            "#include <stdio.h>", "#include <string.h>", "#include <sqludf.h>", "", "void  BLOBLOCINF (int * locator,",
        ...
        try {
          ...
          try {
            s1.executeUpdate("create procedure " + "QGPL.JDCMDEXEC(IN CMDSTR VARCHAR(1024),IN CMDLEN DECIMAL(15,5)) "
          ...
          String sql = " CREATE FUNCTION " + JDLobTest.COLLECTION + ".BLOBLOCINF (int) RETURNS VARCHAR(200) "
              ...
              + "/BLOBLOCINF(BLOBLOCINF)' PARAMETER STYLE SQL";
  ...
  /**
   * getBytes() - Should throw an exception when start is less than 0.
   **/
  public void Var004() {
    if (checkJdbc20()) {
      if (checkLobSupport()) {
  ...
  /**
   * getBytes() - Should return an empty byte array than the length of the lob.
   * 
   * SQL400 - Given what this test is doing, this should be an exception
   * condition.
   **/
  public void Var006() {
  ...
  /**
   * getBytes() - Should throw an exception when length is less than 0.
   **/
  public void Var007() {
    if (checkJdbc20()) {
      if (checkLobSupport()) {
   ...
   * the lob.
   * 
   * SQL400 - The test is saying it will test the right thing but then testing the
   * wrong thing.
   * 
...
//                    assertExceptionIsInstanceOf (e, "java.sql.SQLException");
    ...
    if (checkJdbc20()) {
      if (checkLobSupport()) {
        try {
          ...
          assertCondition(areEqual(v, MEDIUM_));
        } catch (Exception e) {
          failed(e, "Unexpected Exception");
        }
      }
    ...
    if (checkJdbc20()) {
      if (checkLobSupport()) {
        try {
          ...
          assertCondition(areEqual(v, LARGE_));
        } catch (Exception e) {
          failed(e, "Unexpected Exception");
        }
      }
  ...
  /**
   * position(byte[],long) - Should throw an exception when start is greater than
  ...
  /**
   * position(byte[],long) - Should return -1 when the pattern is not found at
   * all.
   **/
  public void Var022() {
    if (checkJdbc20()) {
  ...
  /**
   * position(byte[],long) - Should return 0 when the pattern is found at the
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(byte[],long) - Should return the position when the pattern is found
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(byte[],long) - Should return 0 when the pattern is the empty string
   * and start is 1.
   **/
  public void Var030() {
    if (checkJdbc20()) {
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(Blob,long) - Should throw an exception when start is greater than
   * the length of the lob.
   **/
  public void Var036() {
    if (checkJdbc20()) {
  ...
  /**
   * position(Blob,long) - Should return 0 when the pattern is found at the
   * beginning of the lob.
   **/
  public void Var039() {
    if (checkJdbc20()) {
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(Blob,long) - Should return the position when the pattern is found in
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(Blob,long) - Should return the position when the pattern is found at
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(Blob,long) - Should return 0 when the pattern is the empty lob and
   * start is 1.
   **/
  public void Var045() {
    if (checkJdbc20()) {
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * position(Blob,long) - Should return start when the pattern is the empty lob
...
//                        assertCondition (v == -1); // position() not supported for LOB locators.
  ...
  /**
   * setBytes(long, byte[]) - Should throw an exception when start is less than 0.
   **/
  public void Var050() {
  ...
  /**
   * setBytes(long, byte[]) - Should throw an exception when start is greater than
   * the length of the lob.
   * 
...
//                    }
      ...
      /*
       * } else if (getDriver () == JDTestDriver.DRIVER_TOOLBOX) { //@C4C Because
       * Toolbox only knows the max length of a lob, we will //@C4C not throw an
       * exception if the user asks for a situation where the start //@C4C is greater
       * than the length of the lob until updateRow() is called. try {
       * rs2_=statement2_.executeQuery("SELECT * FROM " + TABLE_); rs2_.absolute (4);
       * Blob blob = rs2_.getBlob ("C_BLOB"); blob.setBytes (SMALL_.length+1, new
       * byte[] { (byte) 44}); rs2_.updateBlob("C_BLOB", blob); rs2_.updateRow();
       * failed ("Didn't throw SQLException"); rs2_.close(); } catch (Exception e) {
       * try { assertExceptionIsInstanceOf (e, "java.sql.SQLException"); rs2_.close();
       * } catch (Exception s) { failed (s, "Unexpected Exception"); } } }
       */
    }
  }

  /**
   * setBytes(long, byte[]) - Should throw an exception when length is less than 0
   * for the number of bytes to write.
   **/
  public void Var053() {
    if (checkUpdateableLobsSupport()) {
      try {
  ...
  // @C2C Changed testcase to not expect an exception if the writing of the bytes
  // will extend
  // @C2C past the end of the lob.
  /**
   * setBytes(long, byte[]) - Should now throw an exception when length of bytes
   * to write extends past the length of the lob as long as start is not passed
   * the end of the lob.
   **/
  public void Var054() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(areEqual(b, expected) && written == 2,
            "written(" + written + ")!= 2 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected));
        rs2_.close();
        connection_.commit();
    ...
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(areEqual(b, expected) && written == 3,
            "written(" + written + ")!= 3 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected)); // @C2C
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
    ...
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(areEqual(b, expected) && written == 1,
            "written(" + written + ")!= 1 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected)); // @C2C
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
  ...
  /**
   * setBytes(long, byte[]) - Should work to set the middle part of a non-empty
   * lob.
   **/
  public void Var057() {
    if (checkUpdateableLobsSupport()) {
  ...
  /**
   * setBytes(long, byte[]) - Should work to set the first part of a non-empty
   * lob.
   **/
  public void Var058() {
    if (checkUpdateableLobsSupport()) {
  ...
  /**
   * setBytes(long, byte[]) - Should throw exception because lob was truncated in
   * var. 58.
   * 
   * @CRS - This variation should succeed. Only when you try to write past the max
   *      column size will it be an exception.
...
//                assertExceptionIsInstanceOf (e, "java.sql.SQLException");
  ...
  /**
   * setBytes(long, byte[], int, int) - Should throw an exception when start is
   * less than 0.
   **/
  public void Var060() {
    if (checkUpdateableLobsSupport()) {
  ...
  // @C2C past the end of the lob.
  /**
   * setBytes(long, byte[], int, int) - Should not throw an exception when length
   * of bytes to write extends past the length lob as long as start is not passed
   * the end of the lob.
   **/
  public void Var063() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(areEqual(b, expected) && written == 4,
            "written(" + written + ")!= 4 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected)); // @C2C
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
  ...
  /**
   * setBytes(long, byte[], int, int) - Should work to set the middle part of a
   * non-empty lob.
   **/
  public void Var065() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(areEqual(b, expected) && written == 2,
            "written(" + written + ")!= 2 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected)); // @C2C
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
  ...
  /**
   * setBytes(long, byte[], int, int) - Should work to set the last part of a
   * non-empty lob.
   **/
  public void Var066() {
    if (checkUpdateableLobsSupport()) {
  ...
  /**
   * setBytes(long, byte[], int, int) - Should work to set the first part of a
   * non-empty lob.
   **/
  public void Var067() {
    if (checkUpdateableLobsSupport()) {
  ...
  /**
   * setBytes(long, byte[], int, int) - Should throw exception because the lob was
   * truncated in var. 67.
   * 
   * @D1: should not expect an exception because expansion is allowed till
   *      maxLength of BLOB
   **/
  public void Var068() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        Blob blob = rs2_.getBlob("C_BLOB");
        byte[] expected = new byte[] { (byte) 578 };
        int written = blob.setBytes(2, expected, 0, 1); // @D1: changed offset from 1 to 0 .... setBytes(2, expected, 1,
                                                        // 1);
        rs2_.updateBlob("C_BLOB", blob);
  ...
  public void Var069() {
    if (checkUpdateableLobsSupport()) {
      try {
        rs2_ = statement2_.executeQuery("SELECT * FROM " + TABLE_);
        /*
         * rs2_.absolute (2); Blob blob2 = rs2_.getBlob ("C_BLOB"); rs2_.absolute (3);
  ...
  /**
   * setBinaryStream() - Should throw an exception if you try to set a stream that
...
//            {
  ...
  /**
   * setBytes(long, byte[]) - Make sure lob in database is not changed if
  ...
  /**
   * setBytes(long, byte[], int, int) - Should work to set bytes to a non-empty
   * lob with multiple updates to the same position.
   **/
  public void Var079() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        Blob blob1 = rs2_.getBlob("C_BLOB");
        byte[] b = blob1.getBytes(1, 3);
        assertCondition(areEqual(b, expected2) && written == 3 && written2 == 3, "written(" + written + ")!= 3 OR "
            + "written2(" + written2 + ")!= 3 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected2));
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
  ...
  /**
   * setBytes(long, byte[], int, int) - Should work to set bytes to a non-empty
   * lob with multiple updates to the different positions.
   **/
  public void Var080() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(areEqual(b, expected) && written == 3 && written2 == 3, "written(" + written + ")!= 3 OR "
            + "written2(" + written2 + ")!= 3 OR " + "b(" + dumpBytes(b) + ")!=" + dumpBytes(expected));
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
  ...
  /**
   * setBytes(long, byte[], int, int) - Should work to set bytes to a non-empty
   * lob with multiple updates to the different positions with updateBlob calls
   * between.
   **/
  public void Var082() {
    if (checkUpdateableLobsSupport()) {
  ...
  public void Var093() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        rs2_.absolute(2);
        Blob blob1 = rs2_.getBlob("C_BLOB");
        assertCondition(length1 == 50 && blob1.length() == 2,
            "length1 = " + length1 + " SB 50, blob1.length() =  " + blob1.length() + " SB 2");
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
  ...
  public void Var094() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        rs2_.absolute(2);
        Blob blob1 = rs2_.getBlob("C_BLOB");
        assertCondition(length1 == 2 && blob1.length() == 30000 && written == 30000, "length1 = " + length1
            + " SB 2, blob1.length() =  " + blob1.length() + " SB 30000, written = " + written + " SB 30000");
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        if (getDriver() == JDTestDriver.DRIVER_TOOLBOX)
...
// Clearing up a few things of previous testcases
        ...
        assertCondition(success && length1 == 2 && length2 == 14, "success = " + success + " SB true. " + "length1 = "
            + length1 + " SB 2, length2 =  " + blob1.length() + " SB 14");
        rs2_.close();
        connection_.commit();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
        ...
        try {
          ...
          for (int i = 0; i < FREE_LOCATOR_BLOCK_SIZE; i++) {
            ...
            ResultSet rs = statement2_.executeQuery(
                "select " + JDLobTest.COLLECTION + ".BLOBLOCINF(" + locator[i] + ")" + " from qsys2.qsqptabl");
            rs.next();
            answer[i] = rs.getString(1);
            rs.close();
          }
          assertCondition(answer[0].indexOf("ERROR") >= 0 && answer[FREE_LOCATOR_BLOCK_SIZE - 1].indexOf("ERROR") >= 0,
              "Check said " + answer[0]);
    ...
    if ((getDriver() != JDTestDriver.DRIVER_TOOLBOX)) {
      if (checkLobSupport() && checkJdbc40()) {
        try {
          ...
          assertCondition(
              (answer[0].indexOf("ERROR") >= 0) && answer[FREE_LOCATOR_BLOCK_SIZE - 1].indexOf("ERROR") >= 0,
              "Check said " + answer[0]);
        }
      ...
      if (checkJdbc40()) {
        try {
          ...
          ResultSet rs = statement2_
              .executeQuery("select " + JDLobTest.COLLECTION + ".BLOBLOCINF(" + locator + ")" + " from qsys2.qsqptabl");
          rs.next();
