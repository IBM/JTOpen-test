...
/**
 * Testcase JDLobBlob. This tests the following method of the JDBC Blob class:
 * 
 * <ul>
 * <li>getBinaryStream()
 * <li>getBytes()
 ...
 * </ul>
 **/
public class JDLobBlob extends JDTestcase {
   ...
   * Constructor.
   **/
  public JDLobBlob(AS400 systemObject, Hashtable<String, Vector<String>> namesAndVars, int runMode,
      FileOutputStream fileOutputStream, String password) {
    super(systemObject, "JDLobBlob", namesAndVars, runMode, fileOutputStream, password);
  }
  ...
  protected void setup() throws Exception {
    ...
    if (isJdbc20()) {
      ...
      statement1_ = connection_.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
      ...
      statement1_.executeUpdate("CREATE TABLE " + TABLE_ + "(C_VARBINARY VARCHAR(" + WIDTH_ + ") FOR BIT DATA)");

      PreparedStatement ps = connection_.prepareStatement("INSERT INTO " + TABLE_ + " (C_VARBINARY) VALUES (?)");
      ps.setString(1, "");
      ps.executeUpdate();
  ...
  /**
   * getBytes() - Should throw an exception when start is greater than the length
   * of the lob.
   * 
   * @D2: Native driver extracts with padding for specified length exceeding
  ...
  public void Var032() {
    if (checkJdbc20()) {
      try {
        ...
        Blob blob = rs_.getBlob("C_VARBINARY");
        byte[] test = new byte[0];
        long v = blob.position(test, MEDIUM_.length);
        assertCondition(v == MEDIUM_.length, "v = " + v + " SB " + MEDIUM_.length);
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
      }
    }
  ...
  public void Var034() {
    if (checkJdbc20()) {
      try {
        ...
        blob.position(new JDLobTest.JDTestBlob(test), -1);
        failed("Didn't throw SQLException");
      } catch (Exception e) {
        assertExceptionIsInstanceOf(e, "java.sql.SQLException");
      }
  ...
  public void Var035() {
    if (checkJdbc20()) {
      try {
        ...
        Blob blob = rs_.getBlob("C_VARBINARY");
        byte[] test = new byte[] { -34, 23, 5, 6 };
        blob.position(new JDLobTest.JDTestBlob(test), 0);
        failed("Didn't throw SQLException");
      } catch (Exception e) {
        assertExceptionIsInstanceOf(e, "java.sql.SQLException");
      }
  ...
  public void Var037() {
    if (checkJdbc20()) {
      try {
        ...
        long v = blob.position(new JDLobTest.JDTestBlob(test), 1);
        assertCondition(v == -1);
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
      }
  ...
  public void Var038() {
    if (checkJdbc20()) {
      try {
        ...
        long v = blob.position(new JDLobTest.JDTestBlob(test), 14);
        assertCondition(v == -1);
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
      }
  ...
  public void Var041() {
    if (checkJdbc20()) {
      try {
        ...
        long v = blob.position(new JDLobTest.JDTestBlob(test), 5);
  ...
  public void Var042() {
    if (checkJdbc20()) {
      try {
        ...
        long v = blob.position(new JDLobTest.JDTestBlob(test), 22);
  ...
  public void Var044() {
    if (checkJdbc20()) {
      try {
        ...
        long v = blob.position(new JDLobTest.JDTestBlob(test), 49);
  ...
  public void Var046() {
    if (checkJdbc20()) {
      try {
        ...
        Blob blob = rs_.getBlob("C_VARBINARY");
        byte[] test = new byte[0];
        long v = blob.position(new JDLobTest.JDTestBlob(test), 45);
  ...
  public void Var047() {
    if (checkJdbc20()) {
      try {
        ...
        Blob blob = rs_.getBlob("C_VARBINARY");
        byte[] test = new byte[0];
        long v = blob.position(new JDLobTest.JDTestBlob(test), MEDIUM_.length);
        assertCondition(v == MEDIUM_.length, "v = " + v + " SB " + MEDIUM_.length);
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
      }
  ...
  public void Var072() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        byte[] expected = new byte[] { (byte) 6, (byte) 10, (byte) 11, (byte) 12 };
        InputStream inputStream = blob1.getBinaryStream();
        sb.setLength(0);
        if (getDriver() == JDTestDriver.DRIVER_NATIVE) { // @D4
    ...
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        assertCondition(length1 == 50 && blob1.length() == 2,
            "length1 = " + length1 + " SB 50 AND blob1.length() = " + blob1.length() + " SB 2");
        rs2.close();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
      }
  ...
  public void Var084() {
    if (checkUpdateableLobsSupport()) {
      try {
        ...
        rs2.absolute(5);
        Blob blob1 = rs2.getBlob("C_VARBINARY");
        assertCondition(length1 == 50 && blob1.length() == 0,
            "length1 = " + length1 + " SB 50 AND blob1.length() = " + blob1.length() + " SB 2");
        rs2.close();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
...
// Pre-cleaning
        ...
        assertCondition(areEqual(b, expected) && written == 3 && blob1.length() == 7);
        rs2.close();
      } catch (Exception e) {
        failed(e, "Unexpected Exception");
      }
