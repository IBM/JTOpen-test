...
//
// Filename:  JDLobClobLocator8.java
//
...
/**
Testcase JDLobClobLocator8.  This tests the following method
...
**/
    public JDLobClobLocator8 (AS400 systemObject,
                             ...
                             String password)
    {
        super (systemObject, "JDLobClobLocator8",
...
**/
    public void Var303()
    {
        ...
        if (checkJdbc20 ()) {
			...
			Clob clob = rs.getClob ("C_CLOB");
			InputStream v = clob.getAsciiStream ();
			assertCondition (compare (v, HUGE_, "8859_1"), "extracted stream not equal to expected String -- clob.length = "+clob.length()+" sb "+HUGE_.length() +" -- Added 11/30/2005 by native");
			stmt.close();
		    }
                }
                catch (Exception e) {
    ...
    public void Var305()
    {
		...
		if (!LONGRUNNING ) {
			...
			if (!rowAvailable) {
			    ...
			    v = clob.getSubString (1, HUGE_WIDTH_ - 2);
    ...
    **/
    protected  boolean compare (InputStream i, byte[] b)
    {
		...
		output_.println("  Extra bytes("+restCount+")="+dumpBytes(buf));
            ...
            return total == b.length && allDone  && areEqual(b, buf);	
...
/*
    public  boolean isEqual(byte[] expected, byte[] actual) {
	int offset = 0; 
