...
/**
 * Testcase JDConnectionLibraries. This tests the following properties with
 * respect to the JDBC Connection class:
 * 
 * <ul>
 ...
 * </ul>
 **/
public class JDConnectionLibraries extends JDTestcase implements TimeoutThreadCallback {
  ...
  private String part1URL_;

  private String part2URL_;

  /**
   * Constructor.
   **/
  public JDConnectionLibraries(AS400 systemObject, Hashtable<String, Vector<String>> namesAndVars, int runMode,
      FileOutputStream fileOutputStream,

      String password, String pwrUserId, String pwrPwd) // @C1C
  {
    super(systemObject, "JDConnectionLibraries", namesAndVars, runMode, fileOutputStream, password);
  ...
  protected void setup() throws Exception {
    ...
    JDSetupCollection.create(systemObject_, c, COLLECTION2, output_);
    c.close();
    ...
    /* Break the baseURL_ into parts */
    int semicolonIndex = baseURL_.indexOf(';');
    if (semicolonIndex > 0) {
      part1URL_ = baseURL_.substring(0, semicolonIndex);
      part2URL_ = baseURL_.substring(semicolonIndex);
   ...
   * @return true if the schema is in the library list, false otherwise.
   **/

  private boolean checkLibraryList(Connection c, String schema, StringBuffer sb) throws Exception {
   ...
   * @param c      The connection.
   * @param schema The schema.
   * @return true if the schema is the default schema, false otherwise.
   * 
   *         SQL400 - The native JDBC driver gets different error messages from
   *         the Toolbox and therefore must examine the error messages for
   *         different values.
   **/
  String getDefaultSchemaErrorMessage = "";

  private String getDefaultSchema(Connection c) throws Exception {
  ...
  /**
   * default schema - No default schema is specified in the URL, no libraries are
   * specified, and sql naming. Should use the schema with the same name as the
   * userId as the default.
   **/
  public void Var001() {
    try {
      Connection c = testDriver_.getConnection(baseURL_ + ";naming=sql", userId_, encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
      assertCondition(defaultSchema.equalsIgnoreCase(userId_),
          "Default schema is '" + defaultSchema + "' should be '" + userId_ + "' URL was " + baseURL_
              + ";naming=sql schema retrieved from '" + getDefaultSchemaErrorMessage + "'");
    } catch (Exception e) {
  ...
  /**
   * default schema - No default schema is specified in the URL, no libraries are
   * specified, and system naming. Should have no default.
   **/
  public void Var002() {
    try {
      Connection c = testDriver_.getConnection(baseURL_ + ";naming=system", userId_, encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();

      assertCondition(defaultSchema.equalsIgnoreCase(""),
          "defaultSchema is '" + defaultSchema + "' sb '' URL was " + baseURL_);
  ...
  /**
   * default schema - Specifies an invalid default schema in the URL. Should throw
   * an exception.
   * 
   * SQL400 - the Native JDBC driver will not validate that the library passed in
   * is a valid library that currently exists on the system. To do so is error
   * prone due to the fact that DRDA has no mechanism to check and return
   * information about a default library at connection time. The native driver
   * will only check that the length of the string passed in is valid.
   * 
   ...
   * example, customers with underscores in their schema names would not work. The
   * code was removed. Now we send whatever we get to the server and let them deal
   * with it.
   * 
   * 
   **/
  public void Var003() {
    ...
    return;
    /*
     * try { Connection c = testDriver_.getConnection (baseURL_ + "/BAD_SCHEMA",
     * userId_, encryptedPassword_);
     * 
     * if (getDriver() == JDTestDriver.DRIVER_NATIVE) { assertCondition(true);
     * c.close(); } else failed ("Did not throw exception.");
     * 
     * } catch (Exception e) { if (getDriver() == JDTestDriver.DRIVER_NATIVE)
     * failed(e, "Unexpected exception"); else assertExceptionIsInstanceOf (e,
     * "java.sql.SQLException"); }
     */
  }

  /**
   * default schema - Specifies an default schema that is longer than 10
   * characters in the URL. Should throw an exception.
   **/
  public void Var004() {
    ...
    try {
      ...
      // @128len
      // now max is 128
      String url = part1URL_ + "/" + JDConnectionTest.SCHEMAS_LEN128 + "EXTRA" + part2URL_;
      c = DriverManager.getConnection(url, userId_, PasswordVault.decryptPasswordLeak(encryptedPassword_));
      ...
      if (isToolboxDriver()) {
        failed("Did not throw exception. for " + c + " url =" + url);
      } else {
        succeeded();
      }
  ...
  /**
   * default schema - Specifies a default schema in the URL. Verifies that this
   * schema is now the default schema.
   **/
  public void Var005() {
    try {
      String url = part1URL_ + "/" + JDConnectionTest.COLLECTION + part2URL_;
      Connection c = testDriver_.getConnection(url, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
  ...
  /**
   * default schema - No default schema is specified in the URL, but libraries are
   * specified (delimited by spaces), and sql naming. Should use the first library
   * as the default.
   **/
  public void Var006() {
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";naming=sql;libraries=" + JDConnectionTest.COLLECTION + " " + COLLECTION2, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
      assertCondition(defaultSchema.equalsIgnoreCase(JDConnectionTest.COLLECTION));
  ...
  /**
   * default schema - No default schema is specified in the URL, but libraries are
   * specified (delimited by commas), and sql naming. Should use the first library
   * as the default.
   **/
  public void Var007() {
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";libraries=" + COLLECTION2 + "," + JDConnectionTest.COLLECTION, userId_, encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
  ...
  /**
   * default schema - No default schema is specified in the URL, but libraries are
   * specified (delimited by spaces), and system naming. Should have no default.
   * 
  ...
  public void Var008() {
    try {
      String thisURL = baseURL_ + ";naming=system;libraries=" + JDConnectionTest.COLLECTION + " " + COLLECTION2;
  ...
  /**
   * default schema - Specifies a default schema and some libraries. Verify that
   * the default schema comes from the URL.
   **/
  public void Var009() {
    try {
      String url = part1URL_ + "/" + JDConnectionTest.COLLECTION + ";libraries=" + "," + COLLECTION2+part2URL_; 
      Connection c = testDriver_.getConnection(
          url, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
  ...
  /**
   * libraries - No default schema is specified in the URL, no libraries are
   * specified. The schema with the same name as the userId should the current
   * schema.
   **/
  public void Var010() {
  ...
  /**
   * libraries - Specifies a default schema in the URL. Verifies that this schema
  ...
  public void Var011() {
    ...
    try {
      String url = part1URL_ + "/" + JDConnectionTest.COLLECTION + ";naming=system"+part2URL_;
      Connection c = testDriver_.getConnection(url, userId_,
          encryptedPassword_);
  ...
  /**
   * libraries - No default schema is specified in the URL, but libraries are
   * specified (delimited by spaces). Those libraries should be in the library
   * list.
   **/
  public void Var012() {
    StringBuffer sb = new StringBuffer();
    ...
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";naming=system;libraries=" + JDConnectionTest.COLLECTION + " " + COLLECTION2, userId_,
          encryptedPassword_);
  ...
  /**
   * libraries - No default schema is specified in the URL, but libraries are
   * specified (delimited by commas). Those libraries should be in the library
   * list.
   **/
  public void Var013() {
    StringBuffer sb = new StringBuffer();
    ...
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";naming=system;libraries=" + COLLECTION2 + "," + JDConnectionTest.COLLECTION, userId_,
          encryptedPassword_);
      boolean successA = checkLibraryList(c, JDConnectionTest.COLLECTION, sb);
      boolean successB = checkLibraryList(c, COLLECTION2, sb);
      c.close();
  ...
  /**
   * libraries - Specifies a default schema and some libraries. Verify that the
   * default schema and the libraries are in the library list.
   **/
  public void Var014() {
    StringBuffer sb = new StringBuffer();

    try {
      String url = part1URL_ + "/" + JDConnectionTest.COLLECTION + ";libraries=" + "," + COLLECTION2 + ";naming=system"+part2URL_;
      Connection c = testDriver_.getConnection(
          url, userId_,
          encryptedPassword_);
      boolean successA = checkLibraryList(c, JDConnectionTest.COLLECTION, sb);
  ...
  /**
   * libraries - Specifies a bad library in the library list. Should throw an
   * exception.
   * 
   * In v5r2 the Toolbox driver was changed to post a warning if a library does
   * not exist or is already in the list.
   **/
  public void Var015() {
    try {
      // Toolbox: in v7r1, if default schema (first in libraries) is greater than
      // length 10, then it is not even added to lib list, and will not get
      // warning....
      Connection c = testDriver_.getConnection(baseURL_ + ";libraries=BAD_LIB", userId_, encryptedPassword_);
      if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
        c.close();
        /**
         * The native driver doen't care about the libraries property unless system
         * naming is used
         */
        succeeded();
      } else {
  ...
  public void Var016() {
    ...
    try {
      Connection c = testDriver_.getConnection(baseURL_ + ";libraries=*LIBL", userId_, encryptedPassword_);
    ...
    try {
      ...
      if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
        ...
        c = testDriver_.getConnection(
            baseURL_ + ";naming=system;libraries=*LIBL," + JDConnectionTest.COLLECTION + "," + COLLECTION2, userId_,
            encryptedPassword_);

      } else {
        c = testDriver_.getConnection(baseURL_ + ";libraries=*LIBL," + JDConnectionTest.COLLECTION + "," + COLLECTION2,
            userId_, encryptedPassword_);
      }
  ...
  public void Var019() {
    ...
    try {
      ...
      if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
        c = testDriver_.getConnection(
            baseURL_ + ";naming=system;libraries=" + JDConnectionTest.COLLECTION + ",*LIBL," + COLLECTION2, userId_,
            encryptedPassword_);
      } else {
        c = testDriver_.getConnection(baseURL_ + ";libraries=" + JDConnectionTest.COLLECTION + ",*LIBL," + COLLECTION2,
            userId_, encryptedPassword_);
      }
      boolean successA = checkLibraryList(c, JDConnectionTest.COLLECTION, sb);
      boolean successB = checkLibraryList(c, COLLECTION2, sb);
  ...
  public void Var020() {
    ...
    try {
      ...
      if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
        c = testDriver_.getConnection(
            baseURL_ + ";naming=system;libraries=" + JDConnectionTest.COLLECTION + "," + COLLECTION2 + ",*LIBL",
            userId_, encryptedPassword_);
      } else {
        c = testDriver_.getConnection(
            baseURL_ + ";libraries=" + JDConnectionTest.COLLECTION + "," + COLLECTION2 + ",*LIBL", userId_,
            encryptedPassword_);
      }
      boolean successA = checkLibraryList(c, JDConnectionTest.COLLECTION, sb);
      boolean successB = checkLibraryList(c, COLLECTION2, sb);
  ...
  /**
   * default schema - No default schema is specified in the URL, no libraries are
   * specified, and sql naming (the default). Should use the schema with the same
   * name as the userId as the default.
   **/
  public void Var021() {
    if ((!isToolboxDriver())) {
  ...
  /**
   * libraries - No default schema is specified in the URL, no libraries are
   * specified. The schema with the same name as the userId should be in the
   * library list.
   **/
  public void Var031() {
    if ((!isToolboxDriver())) {
      ...
      notApplicable("TODO: figure out good way to check library list");// checklibraryList() was hardcoded to return
                                                                       // true!!
      return;
    }
    StringBuffer sb = new StringBuffer();
  ...
  /*
   * default schema - No default schema is specified in the URL, no libraries are
   * specified, and sql naming (default). Should use the schema with the same name
   * as the userId as the default.
   **/
  public void Var044() {
    if ((!isToolboxDriver())) {
      notApplicable("TOOLBOX ONLY VARIATION");
  ...
  /**
   * libraries - Specifies a library already in the library list. Should post a
   * warning.
   * 
   * In v5r2 the Toolbox driver was changed to post a warning if a library does
   * not exist or is already in the list.
  ...
  public void Var045() {
    try {
      Connection c = testDriver_.getConnection(baseURL_ + ";naming=system;libraries=QSYS,QGPL", userId_,
          encryptedPassword_);
      if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
  ...
  /*
   * default schema - No default schema is specified in the URL, libary list
   * starts with common. Result is no default schema. (sql naming (default) so
   * should use the schema with the same name as the userId as the default.)
   **/
  public void Var046() {
    if ((!isToolboxDriver())) {
      notApplicable("TOOLBOX ONLY VARIATION");
  ...
  /*
   * default schema - No default schema is specified in the URL, libary list
   * starts with spaces then a common. Result is no default schema. (sql naming
   * (default) so should use the schema with the same name as the userId as the
   * default.)
   **/
  public void Var047() {
    if ((!isToolboxDriver())) {
  ...
  /*
   * default schema - No default schema is specified in the URL, libary list
   * starts with common. Result is no default schema. System naming so no default.
   **/
  public void Var048() {
    if ((!isToolboxDriver())) {
      notApplicable("TOOLBOX ONLY VARIATION");
  ...
  /*
   * default schema - No default schema is specified in the URL, libary list
   * starts with spaces then a common. Result is no default schema. System naming.
   **/
  public void Var049() {
    if ((!isToolboxDriver())) {
      notApplicable("TOOLBOX ONLY VARIATION");
  ...
  /*
   * Make sure default library is added to library list. Starting with v5r1 the
   * server no longer automatically add the default library to the library list.
   * This broke many ODBC customers and Dave Dilling says it will only be time
   * before we get APARs. So, code was added to add the default library to the
   * library list. We test this by loading a stored procedure. SPs are found by
   * looking in the libary list, not by using the default lib. So, put a SP in the
   * default lib and call it using an unqualified name. If the default lib is not
   * in the library list we will get an error (SP not found). If it is in the
   * library list we will get a result set back.
   **/
  public void Var050() {
    ...
    String SP = "CREATE PROCEDURE " + proc + " (parm1 VARCHAR(20)) " + "RESULT SET 1 LANGUAGE SQL " + "BEGIN  "
        + "   DECLARE C3 CURSOR FOR SELECT * FROM " + table + "  ; " + "   OPEN C3 ; "
        + "   SET RESULT SETS CURSOR C3 ; " + "END  ";

    String setupURL = baseURL_ + ";libraries=QGPL";
    String testURL = part1URL_ + "/" + collectionWithSP + ";libraries=QGPL"+part2URL_;
    ...
    try {
      ...
      try {
        ...
        try {
          setupStatement.executeUpdate("CREATE TABLE " + table + " (p1_int integer)");
        } catch (Exception e) {
        }
        try {
          setupStatement.executeUpdate("INSERT INTO " + table + " VALUES (98765)");
        } catch (Exception e) {
        }
        try {
          setupStatement.executeUpdate(SP);
        } catch (Exception e) {
          output_.println(e);
        }
      } catch (Exception e) {
...
//         try { statement.close(); } catch (Exception e) { }
    ...
    if (setupStatement != null) {
      try {
        setupStatement.executeUpdate("DROP TABLE " + table);
      } catch (Exception e) {
      }
      try {
        setupStatement.executeUpdate("DROP PROCEDURE " + proc);
      } catch (Exception e) {
      }
      // try { setupStatement.executeUpdate("DROP COLLECTION " + collectionWithSP); }
      // catch (Exception e) { }
      try {
...
//	   Statement  statement  = null;
    ...
    String testURL = part1URL_ + "/" + collectionWithSP + ";libraries=QGPL;naming=system"+part2URL_;
...
//Note:  you need SECOFR authority to issue a change query library command
    ...
    if ((isToolboxDriver()) || (getDriver() == JDTestDriver.DRIVER_NATIVE)) {
      ...
      try {
        if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
          ...
          try {
            command = "CALL QSYS.QCMDEXC(" + "'CRTLIB " + iniLibrary + "                                     ',"
                + "0000000040.00000 )";
          ...
          try {
            command = "CALL QSYS.QCMDEXC('CRTDUPOBJ OBJ(QAQQINI) FROMLIB(QSYS) OBJTYPE(*FILE) TOLIB(" + iniLibrary
                + ") DATA(*YES)    ',0000000080.00000 )";
            s2.executeUpdate(command);
          } catch (Exception e) {
      ...
      try {
        if (setupFailed == true)
          ...
          timeoutThread.abort();
          timeoutThread.join();
          failed("Didn't throw SQLException after setting qaqqinilib property for query time limit=1:  Got result set"
              + rs);
        }
      } catch (Exception e) {
        ...
        try {
          ...
          if (getDriver() == JDTestDriver.DRIVER_NATIVE) {
            Connection c = testDriver_.getConnection(baseURL_, pwrSysUserID_, pwrSysEncryptedPassword_);
            Statement s2 = c.createStatement();
            s2.executeUpdate("CALL QSYS.QCMDEXC(" + "'CHGJOB INQMSGRPY(*SYSRPYL)                                   ',"
                + "0000000040.00000 )");
  ...
  public void Var053() {
    try {
      ...
      String url = part1URL_ + "/" + JDConnectionTest.SCHEMAS_LEN128+part2URL_;
      Connection c = testDriver_.getConnection(url, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
      assertCondition(compareLongSchemas(JDConnectionTest.SCHEMAS_LEN128, defaultSchema),
          "future host support required."); // note only returned as 10 chars
    } catch (Exception e) {
      failed(e, "Unexpected exception");
    }
  }
  ...
  // hostserver returns something like JDTES00001 in message back for long schema
  // like JDTESTCOTHISISAREALLY
  ...
  public void Var054() {
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";naming=sql;libraries=" + JDConnectionTest.SCHEMAS_LEN128 + " " + COLLECTION2, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
      assertCondition(compareLongSchemas(JDConnectionTest.SCHEMAS_LEN128, defaultSchema),
  ...
  public void Var055() {
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";libraries=" + JDConnectionTest.SCHEMAS_LEN128 + "," + JDConnectionTest.COLLECTION, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
      assertCondition(compareLongSchemas(defaultSchema, JDConnectionTest.SCHEMAS_LEN128),
          "future host support required."); // note only returned as 10 chars
    } catch (Exception e) {
      failed(e, "Unexpected Exception");
    }
  ...
  public void Var056() {
    try {
      Connection c = testDriver_.getConnection(
          baseURL_ + ";naming=system;libraries=" + JDConnectionTest.SCHEMAS_LEN128 + " " + COLLECTION2, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
      c.close();
      boolean condition = defaultSchema.equalsIgnoreCase("");
  ...
  public void Var057() {
    try {
      String url = part1URL_ + "/" + JDConnectionTest.SCHEMAS_LEN128 + ";libraries=" + "," + COLLECTION2+part2URL_;
      Connection c = testDriver_.getConnection(
          url, userId_,
          encryptedPassword_);
      String defaultSchema = getDefaultSchema(c);
  ...
  /**
   * libraries - Specifies a default schema in the URL. Verifies that this schema
   * is in the library list for system naming
   **/
  public void Var058() {
    StringBuffer sb = new StringBuffer();

    try {
      String url =  part1URL_ + "/" + JDConnectionTest.SCHEMAS_LEN128 + ";naming=system"+part2URL_;
      Connection c = testDriver_.getConnection(url,
          userId_, encryptedPassword_);
