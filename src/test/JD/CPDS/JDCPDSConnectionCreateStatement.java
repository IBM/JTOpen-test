...
/**
Testcase JDCPDSConnectionCreateStatement.  This tests the following methods
...
**/
public class JDCPDSConnectionCreateStatement
extends JDTestcase {
    ...
    // java.sql.Connection for the connection_ object, since the createStatement(int,int)
...
Constructor.
**/
    public JDCPDSConnectionCreateStatement (AS400 systemObject,
                                            ...
                                            String password)
    {
        super (systemObject, "JDCPDSConnectionCreateStatement",
...
**/
    ...
    throws Exception
    {
		...
		s.executeUpdate ("CREATE TABLE " + keysTable_
				 + " (NAME VARCHAR(10), GENID INT GENERATED ALWAYS AS IDENTITY)");
		s.execute ("INSERT INTO " + keysTable_ + " (NAME) VALUES ('first')", 
...
Now, it correctly throws SQLException
**/
    ...
    public void Var003()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                Statement s = (Statement)connection_.createStatement (ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                int resultSetType = s.getResultSetType ();
                int resultSetConcurrency = s.getResultSetConcurrency ();
...
**/
    public void Var004()
    {
        if (checkJdbc20StdExt()) {
            try {
                Statement s = (Statement) connection_.createStatement (4378,
...
**/
    public void Var007()
    {
        if (checkJdbc20StdExt()) {
            try {
                connection_.clearWarnings ();
                Statement s = (Statement) connection_.createStatement (ResultSet.TYPE_FORWARD_ONLY,
    ...
    public void Var011()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                rs.close ();
                s.close ();
		if (getRelease() >= JDTestDriver.RELEASE_V5R2M0 && getDriver() == JDTestDriver.DRIVER_NATIVE) //@C1
		{
		    boolean condition = ((w == null) 
                        && (type == ResultSet.TYPE_SCROLL_SENSITIVE)
                        && (concurrency == ResultSet.CONCUR_READ_ONLY)
                        && (updatable == false)
                        && (scrollable == true));
        ...
        if (checkJdbc20StdExt()) {
            try {
                PreparedStatement ps = (PreparedStatement)
                                          connection_.prepareStatement (null, ResultSet.TYPE_SCROLL_INSENSITIVE,
                                                                        ResultSet.CONCUR_UPDATABLE);
                failed ("Did not throw exception but got "+ps);
            } catch (Exception e) {
                assertExceptionIsInstanceOf (e, "java.sql.SQLException");
...
**/
    public void Var020()
    {
        if (checkJdbc20StdExt()) {
            try {
                PreparedStatement ps = (PreparedStatement)
                                          connection_.prepareStatement ("", ResultSet.TYPE_SCROLL_INSENSITIVE,
                                                                        ResultSet.CONCUR_READ_ONLY);
                failed ("Did not throw exception but got "+ps);
            } catch (Exception e) {
                assertExceptionIsInstanceOf (e, "java.sql.SQLException");
...
**/
    public void Var022()
    {
        if (checkJdbc20StdExt()) {
            try {
                PreparedStatement ps = (PreparedStatement)
                                          connection_.prepareStatement ("CLIF", ResultSet.TYPE_SCROLL_INSENSITIVE,
                                                                        ResultSet.CONCUR_READ_ONLY);
                failed ("Did not throw exception but got "+ps);
            } catch (Exception e) {
                assertExceptionIsInstanceOf (e, "java.sql.SQLException");
...
**/
    public void Var027()
    {
        if (checkJdbc20StdExt()) {
            try {
                connection_.clearWarnings ();
                PreparedStatement s = (PreparedStatement) connection_.prepareStatement ("SELECT * FROM " + table_ + " FOR UPDATE",
                                                                                              ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
                SQLWarning w = connection_.getWarnings ();
                ResultSet rs = (ResultSet) s.executeQuery ();
                int type = rs.getType ();
    ...
    public void Var028()
    {
        if (checkJdbc20StdExt()) {
            try {
                ...
                rs.close ();
                s.close ();
		if (getRelease() >= JDTestDriver.RELEASE_V5R2M0 && getDriver() == JDTestDriver.DRIVER_NATIVE) //@C2
		{
                assertCondition ((w == null) 
                        && (type == ResultSet.TYPE_FORWARD_ONLY)
                        && (concurrency == ResultSet.CONCUR_UPDATABLE)
...
**/
    public void Var034()
    {
        if (checkJdbc20StdExt()) {
            try {
                CallableStatement cs = (CallableStatement) connection_.prepareCall ("CALL " + JDSetupProcedure.STP_CSPARMS + " (?,?,?)");
...
**/
    public void Var055()
    {
		...
		System.gc();
                ...
                assertCondition (beforeHandle == afterHandle, "Statement Handle Not Reused.  before="+beforeHandle+" after="+afterHandle);
...
**/
    ...
    public void Var056() {
        ...
        try {
            ...
            PreparedStatement ps = connection_.prepareStatement ("INSERT INTO " + keysTable_
		...
		nextWorkedAgain = rs.next();
            ...
            assertCondition (rs != null && key.length() == 1 && nextWorked && !nextWorkedAgain,
			     " rs = "+rs+" should not be null;"+
			     " key.length = "+key.length()+" sb 1;"+
			     " nextWorked = "+nextWorked+" sb true;" +
			     " nexWorkedAgain="+nextWorkedAgain+"sb false "+
