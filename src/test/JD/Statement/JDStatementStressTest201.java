...
//
// Filename:  JDStatementStressTest201.java
//
...
**/
public class JDStatementStressTest201
extends JDStatementStressTest
{
  public static void main(String args[]) throws Exception {
    String[] newArgs = new String[args.length+2];
     newArgs[0] = "-tc";
     newArgs[1] = "JDStatementStressTest201";
...
Constructor.
**/
    public JDStatementStressTest201 (AS400 systemObject,
                        ...
                        String miscParm)
    {
        super (systemObject, "JDStatementStressTest201",
  ...
  public void Var220() {     notApplicable("Moved to JDStatementStressTest206");   }
    ...
    **/
        public void Var201()
        {
            String URL = baseURL_ + ";errors=full;lazy close=true";
            String userid = systemObject_.getUserId(); 
            char[] encryptedPassword =  encryptedPassword_;
            String updateTable = collection_+".JDSSU201"; 

            String ustNumString = System.getProperty("JDStatementStressTest.ustNum");
            if (ustNumString != null) { 
               int newValue = Integer.parseInt( ustNumString); 
               if (newValue > 0) { 
                 ustNum = newValue; 
               }
            }
	    ...
	    /* 
            ...
            */ 
            stmt.executeUpdate("CREATE OR REPLACE TABLE "+updateTable+"( VALUE VARCHAR(80), KEY INT)");
            PreparedStatement ps = connection_.prepareStatement("INSERT INTO "+updateTable+" VALUES(?,?)"); 
            for (int i = 0; i < ustNum * keyBlock; i++) {
            ...
            String updateStatement = "UPDATE "+updateTable+" SET VALUE = ? WHERE KEY = ? ";
            
                JDStatementStressUpdate ust[] = new JDStatementStressUpdate[ustNum];
                for(int i=1; i<=ust.length; i++) {
                    ust[i-1] = new JDStatementStressUpdate(testDriver_, URL, userid, encryptedPassword,
                          i, runMillis_, updateStatement, (i-1)*keyBlock, i*keyBlock-1);
                    ust[i-1].setName("JDStatementStressUpdate" + i);
                }
                

                /* let the threads get set up */ 
                Thread.sleep(1000);   
                for(int i=1; i<=ust.length; i++) {
                  ust[i-1].start();  
                }
                
                while (isThreadStillAlive(ust, null, null)) {
                    Thread.sleep(100);
                }
