...
/**
 * Testcase JDRSAfterLast. This tests the following methods of the JDBC
 * ResultSet classes:
 * 
 * <ul>
 * <li>afterLast()
 ...
 * </ul>
 **/
public class JDRSAfterLast extends JDTestcase {
   ...
   * Constructor.
   **/
  public JDRSAfterLast(AS400 systemObject, Hashtable<String, Vector<String>> namesAndVars, int runMode,
      FileOutputStream fileOutputStream,

      String password) {
    super(systemObject, "JDRSAfterLast", namesAndVars, runMode, fileOutputStream, password);
  }
  ...
  protected void setup() throws Exception {
    ...
    if (isJdbc20()) {
      ...
      connectionNoPrefetch_ = testDriver_.getConnection(baseURL_ + ";data truncation=true;prefetch=false", userId_,
          encryptedPassword_, "JDRSAfterLast2");
  ...
  public void Var001() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS + " WHERE ID = -1");
        rs.afterLast();
  ...
  public void Var002() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS);
        rs.close();
        rs.afterLast();
   ...
   * 
   * SQL400 - We can't support scrollable metadata resultsets (CLI/DB
   * restriction).
   **/
  public void Var008() {
    if (isToolboxDriver() && !isSysibmMetadata()) {
      if (checkJdbc20()) {
        try {
          ResultSet rs = dmd_.getColumns(null, "QIWS", "QCUSTCDT", "%");
          rs.afterLast();
          rs.previous();
  ...
  public void Var014() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement_.executeQuery("SELECT ID, VALUE FROM " + JDRSTest.RSTEST_POS);
        rs.last();
        rs.previous();
        rs.afterLast();
        rs.previous();
  ...
  public void Var017() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement_.executeQuery("SELECT ID, VALUE FROM " + JDRSTest.RSTEST_POS + " FOR UPDATE");
        rs.absolute(-43);
        rs.moveToInsertRow();
        rs.afterLast();
        rs.previous();
  ...
  /**
   * afterLast() - Should work after a moveToInsertRow(), then moveToCurrentRow().
   **/
  public void Var018() {
    if (checkJdbc20()) {
      try {
  ...
  /**
   * afterLast() - Set max rows and fetch the rows using afterLast(). Max rows
   * should not affect this. Native JDBC honors the max rows setting on all cursor
   * positioning methods. The last row in this case has the value 50.
   **/
  public void Var020() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement2_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS);
        rs.afterLast();
        rs.previous();
        int id1 = rs.getInt(1);
        rs.close();
  ...
  public void Var021() {
    if (checkJdbc20()) {
      ...
      try {
        ...
        // Update each value.
        ResultSet rs = statement_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS + " FOR UPDATE OF VALUE");
        PreparedStatement ps = connection_.prepareStatement(
            "UPDATE " + JDRSTest.RSTEST_POS + " SET VALUE = 'AFTERLAST' WHERE CURRENT OF " + rs.getCursorName());
        rs.afterLast();
        rs.previous();
        ps.execute();
        rs.close();
        ...
        ResultSet rs2 = statement_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS);
        for (int i = 1; i <= 99; ++i)
          rs2.next();
...
//            has been closed.
  ...
  /**
   * isAfterLast() - Should return false on an empty result set with no
   * positioning.
   **/
  public void Var022() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return false on a 1 row result set when positioned on
   * the only row.
   **/
  public void Var027() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return false on a large result set when not after the
   * last row.
   **/
  public void Var029() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return true on a large result set when positioned
   * after the last row.
   **/
  public void Var030() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return false on a "simple" result set when not after
   * the last row.
   **/
  public void Var031() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return true on a "simple" result set when positioned
   * after the last row.
   **/
  public void Var032() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return false on a "mapped" result set when not after
   * the last row.
   * 
   * SQL400 - We can't support scrollable metadata resultsets (CLI/DB
   * restriction).
  ...
  /**
   * isAfterLast() - Should return true on a "mapped" result set when positioned
   * to the last row.
   * 
   * SQL400 - We can't support scrollable metadata resultsets (CLI/DB
   * restriction).
  ...
  /**
   * isAfterLast() - Should return false after an absolute(+) to a row other than
   * after the last.
   **/
  public void Var037() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return true after an absolute(+) to after the last
   * row.
   **/
  public void Var038() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return true after an absolute(-) to after the last
   * row. (It is not really possible to test this variation!)
   **/
  public void Var040() {
    if (checkJdbc20()) {
      try {
  ...
  /**
   * isAfterLast() - Should return false after a relative(+) to a row other than
   * after the last.
   **/
  public void Var041() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return true after a relative(+) to after the last row.
   **/
  public void Var042() {
    if (checkJdbc20()) {
      try {
  ...
  /**
   * isAfterLast() - Should return false after a next() when not after the last
   * row.
   **/
  public void Var044() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return true after a next() when positioned after the
   * last row.
   **/
  public void Var045() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return false after a previous() when not after the
   * last row.
   **/
  public void Var046() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() - Should return false after a moveToInsertRow(), then
   * moveToCurrentRow() when not after the last row.
   **/
  public void Var050() {
    if (checkJdbc20()) {
      try {
  ...
  /**
   * isAfterLast() - Should return true after a moveToInsertRow(), then
   * moveToCurrentRow() when after the last row.
   **/
  public void Var051() {
    if (checkJdbc20()) {
      try {
  ...
  /**
   * isAfterLast() - Set max rows and fetch the rows using next(), Verify that max
   * rows is honored.
   **/
  public void Var052() {
    if (checkJdbc20()) {
  ...
  /**
   * isAfterLast() -- Verify that isAfterLast does not move the cursor when called
   * before the beginning of a result set row. This was a bug in the native driver
   * reported by APAR SE20242
   **/
  public void Var053() {
    if (checkJdbc20()) {
      try {
        ...
        rs.close();
        assertCondition((success == false) && (fetched == true),
            "afterLast returned " + success + " sb false and next returned " + fetched
                + " sb true -- added by native driver (08/03/2005) for APAR SE20242");
      } catch (Exception e) {
  ...
  /**
   * isAfterLast() - Should return false after a first() even if the result set is
   * empty.
   **/
  public void Var054() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS + " where 0=1");
        rs.first();
        boolean success = rs.isAfterLast();
        rs.close();
        assertCondition(success == false,
            "isAfterLast() should return false for an empty result Set -- Added by native driver 08/03/2005");
      } catch (Exception e) {
        failed(e, "Unexpected Exception -- added by native driver (08/03/2005)");
      }
    }
  ...
  /**
   * isAfterLast() -- Should return false where there is still more data. Also
   * verify that the result set is correct when isAfterLast is used when prefetch
   * is off
   **/
      ...
      if (checkNative()) {
        ...
        try {
          ...
          try (ResultSet rs = statementNoPrefetch_.executeQuery(sql)) {
            ...
            if (!fetch1) {
              passed = false;
              sb.append("fetch1 returned " + fetch1 + "\n");
            }

            boolean afterLast1 = rs.isAfterLast();
            if (afterLast1) {
              passed = false;
              sb.append("afterLast1 returned " + afterLast1 + "\n");
            }

            String string1 = rs.getString(1);
            if (!"A".equals(string1)) {
              passed = false;
              sb.append("Expected 'A' got '" + string1 + "'\n");
            }

            boolean fetch2 = rs.next();
            if (!fetch2) {
              passed = false;
              sb.append("fetch2 returned " + fetch2 + "\n");
            }

            boolean afterLast2 = rs.isAfterLast();
            if (afterLast2) {
              passed = false;
              sb.append("afterLast2 returned " + afterLast2 + "\n");
            }

            String string2 = rs.getString(1);
            if (!"B".equals(string2)) {
              passed = false;
              sb.append("Expected 'B' got '" + string2 + "'\n");
            }

            boolean fetch3 = rs.next();
            if (!fetch3) {
              passed = false;
              sb.append("fetch3 returned " + fetch3 + "\n");
            }

            boolean afterLast3 = rs.isAfterLast();
            if (afterLast3) {
              passed = false;
              sb.append("afterLast3 returned " + afterLast3 + "\n");
            }

            String string3 = rs.getString(1);
            if (!"C".equals(string3)) {
              passed = false;
              sb.append("Expected 'C' got '" + string3 + "'\n");
            }

            boolean fetch4 = rs.next();
            if (fetch4) {
              passed = false;
              sb.append("fetch4 returned " + fetch4 + "\n");
            }

            boolean afterLast4 = rs.isAfterLast();
            if (!afterLast4) {
              passed = false;
              sb.append("afterLast4 returned " + afterLast4 + "\n");
            }
          }
...
//
  ...
  public void Var061() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statementNoPrefetch_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS + " WHERE ID = -1");
        rs.afterLast();
        boolean success = rs.next();
        rs.close();
        assertCondition(success == false);
  ...
  public void Var062() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statementNoPrefetch_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS);
        rs.close();
        rs.afterLast();
        failed("Didn't throw SQLException");
      } catch (Exception e) {
  ...
  public void Var068() {
    if (isToolboxDriver() && !isSysibmMetadata()) {
      if (checkJdbc20()) {
        try {
          ResultSet rs = dmdNoPrefetch_.getColumns(null, "QIWS", "QCUSTCDT", "%");
          rs.afterLast();
          rs.previous();
          String s1 = rs.getString("TABLE_NAME");
          rs.close();
  ...
  public void Var074() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statementNoPrefetch_.executeQuery("SELECT ID, VALUE FROM " + JDRSTest.RSTEST_POS);
        rs.last();
        rs.previous();
        rs.afterLast();
        rs.previous();
  ...
  public void Var077() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statementNoPrefetch_
            .executeQuery("SELECT ID, VALUE FROM " + JDRSTest.RSTEST_POS + " FOR UPDATE");
        rs.absolute(-43);
        rs.moveToInsertRow();
        rs.afterLast();
        rs.previous();
  ...
  public void Var080() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statement2NoPrefetch_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS);
        rs.afterLast();
        rs.previous();
        int id1 = rs.getInt(1);
        rs.close();
  ...
  public void Var081() {
    if (checkJdbc20()) {
      ...
      try {
        ...
        // Update each value.
        ResultSet rs = statementNoPrefetch_
            .executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS + " FOR UPDATE OF VALUE");
        PreparedStatement ps = connectionNoPrefetch_.prepareStatement(
            "UPDATE " + JDRSTest.RSTEST_POS + " SET VALUE = 'AFTERLAST' WHERE CURRENT OF " + rs.getCursorName());
        rs.afterLast();
        rs.previous();
        ps.execute();
        ...
        // Go through the result set again as a check.
        // It is okay to just use next() here.
        ResultSet rs2 = statementNoPrefetch_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS);
        for (int i = 1; i <= 99; ++i)
          rs2.next();
        String s1 = rs2.getString(2);
        rs2.close();
...
//            has been closed.
  ...
  public void Var114() {
    if (checkJdbc20()) {
      try {
        ResultSet rs = statementNoPrefetch_.executeQuery("SELECT * FROM " + JDRSTest.RSTEST_POS + " where 0=1");
        rs.first();
        boolean success = rs.isAfterLast();
        rs.close();
        assertCondition(success == false,
