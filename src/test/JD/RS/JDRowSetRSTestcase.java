...
**/
    public JDRowSetRSTestcase (AS400 systemObject,
                            ...
                            String pwrPassword)
    {
        super (systemObject, "JDRowSetRSTestcase",
...
**/
        ...
        throws Exception
    {
	   ...
	   if (JTOpenTestEnvironment.isOS400 || JTOpenTestEnvironment.isWindows || JTOpenTestEnvironment.isLinux || JTOpenTestEnvironment.isAIX) {
    ...
    RowSet updater() throws Exception
    {
        ...
        {   //@A1 The native driver's rowset can not execute a command that does not return a result set.
...
**/
    private boolean compareBlobToByteArray (Blob i, byte[] b)
    throws SQLException
    {
    ...
    private boolean compareNClobToString (Object i, String b)
    throws SQLException
    {
        try{
            //same as compareClobToString, but first checks to be sure it is of type NClob (not just Clob)
...
@exception SQLException If an exception occurs.
**/
   private boolean compareClobToString (Clob i, String b)
   throws SQLException
   {
   ...
   public boolean compareColumns(int i, Object[] before, Object[] after,
                                 boolean[] beforeNulls, boolean[] afterNulls, StringBuffer message) {
	...
	if (before[i] == null) {
		...
		if ( before[i] instanceof byte[]) {
			...
			if (afterBytes.length == beforeBytes.length) {
			    ...
			    message.append("For column "+i+" before = "+before[i]+" l="+beforeBytes.length+" after = "+after[i] +" l= "+afterBytes.length+"\n"); 
			    success = false;
			}
		    } else {
		    ...
		    message.append("For column "+i+" before = "+before[i]+" class("+before[i].getClass().getName()+") after = "+after[i] +"\n"); 
		    success = false;
		}
	    }
	}
...
/**
updateRow() - Should update exactly 1 column when updates are
pending.
**/
    public void Var001()
    {
...
**/
   ...
   PasswordVault.clearPassword(charPassword);
               ...
               if (rowset.next ())
               {
                   ...
                   //rowset.updateString ("VALUE", "Here is a lot more than 25 characters.");       //@C1D
...
**/
    ...
    **/
    public void Var007()
    {
        if (getDriver () == JDTestDriver.DRIVER_TOOLBOX)
        {
           //  notApplicable("This testcase fails for some and not for others.  Get back to it when we have free time.");
             //return;  
            //Note:  Paul can never get this to fail...not sure why others see it fail...
        }
       sb.setLength(0);
       String key = "JDRSDeleteRow";
       try
       {
          RowSet rowset = updater(key);
          for (int i = 1; i < 198; ++i)
          ...
          for (int i = 0; i < rowCountAfter; ++i) {                                               //@K1C changed from rowCountBefore to rowCountAfter
              ...
              try                                                                                  //put the try catch in to count for entries with that are null, this occurs if you run   //@K1A
              {                                                                                    //the entire ResultSet testcases.                                                        //@K1A
                  if (! keysBefore.elementAt (i+1).equals (keysAfter.elementAt (i))) {
                    success = false;
                    sb.append("for i="+i+" keysBefore.elementAt (i+1)='"+
                        keysBefore.elementAt (i+1)+"' keysAfter.elementAt(i)=' "+
                        keysAfter.elementAt (i)+"'"); 
                  }
              }                                                                                         //@K1A
              ...
              {                                                                                         //@K1A
                  ...
                  {                                                                                     //@K1A
                      if(!((keysBefore.elementAt(i+1) == null) && 
                           (keysAfter.elementAt(i)==null))) {     //@K1A   
                          success = false;                                                              //@K1A
                          sb.append("\nfor i="+i+" keysBefore.elementAt (i+1)='"+
                              keysBefore.elementAt (i+1)+"' keysAfter.elementAt(i)=' "+
                              keysAfter.elementAt (i)+"'"); 
...
**/
   ...
   PasswordVault.clearPassword(charPassword);
           ...
           JDRSTest.position (rowset, "JDRSInsertRow");
           int columnCount = rowset.getMetaData ().getColumnCount ();
           ...
           assertCondition (rowset.getString ("C_VARCHAR_50").equals ("Space people"), "Did not get expected string column count="+columnCount);
           rowset.close();
        }
        catch (Exception e) {
            failed (e, "Unexpected Exception");
...
**/
   ...
   **/
   ...
   PasswordVault.clearPassword(charPassword);
         ...
         assertCondition(rowset.getCommand().equals(command));
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception");
   ...
   /**
   *  Validates that getConcurrency returns as expected before a result set is created.
   **/
   public void Var060()
   {
   ...
   PasswordVault.clearPassword(charPassword);
         ...
         assertCondition(rowset.getConcurrency() == concurrency);
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception");
   ...
   **/
   public void Var063()
   {
      try
      {
         ...
         assertCondition(rowset.getDataSourceName().equals(jndiName_));
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception");
   ...
   **/
   ...
   PasswordVault.clearPassword(charPassword);
         ...
         assertCondition(rowset.getEscapeProcessing() == false);
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception");
   ...
   **/
   ...
   /**
   *  Validates that getFetchSize returns as expected if a statement has been created.
   **/
   public void Var070()
   {
      RowSet rowset = null;
   ...
   **/
   public void Var076()
   {
      try
      {
         ...
         {
             assertCondition(rowset.getPassword().equals("MYPWD"));
         }
      }
      catch (Exception e)
      {
   ...
   **/
   ...
   PasswordVault.clearPassword(charPassword);
         ...
         assertCondition(rowset.getStatement() != null);
      }
      catch (Exception e)
      {
          if (getDriver () == JDTestDriver.DRIVER_NATIVE)
   ...
   **/
   ...
   PasswordVault.clearPassword(charPassword);
         ...
         assertCondition(rowset.getType() == type);
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception.");
   ...
   **/
   public void Var084()
   {
      ...
      try
      {
         ...
         assertCondition(rowset.getUrl().equals(url));
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception.");
   ...
   **/
   public void Var085()
   {
      ...
      try
      {
         ...
         assertCondition(rowset.getUsername().equals(name));
      }
      catch (Exception e)
      {
         failed(e, "Unexpected Exception.");
    ...
    /**
    *  Validates that rowDeleted() - Should return true on the deleted row when called immediately without repositioning.
    **/
    public void Var086()
    {
       String key = "JDRSRowDeleted";
        try
        {
           RowSet rowset = updater(key);
           rowset.next();
   ...
   /**
   *  Validates that rowInserted() - Should return false on all rows in the updated result set after repositioning.
   **/
   public void Var087()
   {
      String key = "JDRSRowInserted";
       try
       {
          RowSet rowset = updater(key);
          rowset.moveToInsertRow ();
   ...
   /**
   *  Validates that rowUpdated() - Should return false on all rows in the updated result set after repositioning.
   **/
   public void Var088()
   {
      String key = "JDRSRowUpdated";
       try
       {
          RowSet rowset = updater(key);
          rowset.next();
   ...
   **/
   ...
   /**
   *  Validates that moveToCurrentRow() - Should return to the current row after a moveToInsertRow(),
   *  when the current row is positioned using first().
   **/
   public void Var090 ()
   {
      String key = "JDRSMoveToCurrentRow";
      try
      {
         RowSet rowset = getRowSet(jndiName_);
         rowset.setUsername(systemObject_.getUserId());
...
**/
   ...
   /**
   *  Validate that updateAsciiStream() - Should work when the column index is valid.
   **/
    public void Var091()
    {
       String key_ = "JDRSUpdateAsciiStream";

        if (checkJdbc20 ()) {
        try {
           RowSet rowset = updater(key_);
           position (rowset, key_);
           rowset.updateAsciiStream (14, stringToAsciiStream ("Dublin"), 6);
           rowset.updateRow ();
           rowset.close();
    ...
    /**
    *  Validates that updateAsciiStream() - Should work when the column name is valid.
    **/
    public void Var092()
    {
        String key_ = "JDRSUpdateAsciiStream";
        ...
        try {
           ...
           // Update
           position (rowset, key_);
           rowset.updateAsciiStream ("C_VARCHAR_50", stringToAsciiStream ("Ibiza"), 5);
           rowset.updateRow ();
           rowset.close();
...
**/
    public void Var093()
    {
       String key_ = "JDRSUpdateBigDecimal";
...
**/
    public void Var094()
    {
       String key_ = "JDRSUpdateBigDecimal";
        ...
        try {
           ...
           // Update.
           position (rowset, "JDRSUpdateBigDecimal");
           ...
           // Validate.
           RowSet validater = validater();
           position (validater, "JDRSUpdateBigDecimal");
   ...
   /**
   *  Validates that updateBinaryStream() - Should work when the column index is valid.
   **/
    public void Var095()
    {
       String key_ = "JDRSUpdateBinaryStream";
   ...
   /**
   *  Validates that updateBinaryStream() - Should work when the column name is valid.
   **/
    public void Var096()
    {
       String key_ = "JDRSUpdateBinaryStream";
...
**/
    public void Var103()
    {
       String key_            = "JDRSUpdateCharacterStream";
...
**/
    public void Var125()
    {
       String key_            = "JDRSUpdateTimestamp";
...
**/
    ...
    /**
    *  Validates that close() works as expected after and execute without a result set.
    **/
    public void Var127()
    {
       try
   ...
   PasswordVault.clearPassword(charPassword);
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateAsciiStream(int,AsciiStream).
    **/
    public void Var128()
    {
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateAsciiStream(String,AsciiStream).
    **/
    public void Var129()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBigDecimal(int,BigDecimal).
    **/
    public void Var130()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBigDecimal(String,BigDecimal).
    **/
    public void Var131()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBinaryStream(int,BinaryStream).
    **/
    public void Var132()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBinaryStream(String,BinaryStream).
    **/
    public void Var133()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBoolean(int,boolean).
    **/
    public void Var134()
    {
       String key_ = "RowSetListener";
    ...
    **/
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBoolean(String,boolean).
    **/
    public void Var136()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateByte(int,byte).
    **/
    public void Var137()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateByte(String,byte).
    **/
    public void Var138()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBytes(int,byte[]).
    **/
    public void Var139()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateBytes(String,bytes[]).
    **/
    public void Var140()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateCharacterStream(int,CharacterStream).
    **/
    public void Var141()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateCharacterStream(String,CharacterStream).
    **/
    public void Var142()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateDouble(int,double).
    **/
    public void Var143()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateDouble(String,double).
    **/
    public void Var144()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateDate(int,Date).
    **/
    public void Var145()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateDate(String,Date).
    **/
    public void Var146()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateInt(int,int).
    **/
    public void Var147()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateInt(String,int).
    **/
    public void Var148()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateFloat(int,float).
    **/
    public void Var149()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateFloat(String,float).
    **/
    public void Var150()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateLong(int,long).
    **/
    public void Var151()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateLong(String,long).
    **/
    public void Var152()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateNull(int).
    **/
    public void Var153()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateNull(String).
    **/
    public void Var154()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateObject(int,Object).
    **/
    public void Var155()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateObject(String,Object).
    **/
    public void Var156()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateShort(int,short).
    **/
    public void Var157()
    {
       String key_ = "RowSetListener";
    ...
    **/
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateShort(String,short).
    **/
    public void Var159()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateString(int,String).
    **/
    public void Var160()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateString(String,String).
    **/
    public void Var161()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateTime(int,Time).
    **/
    public void Var162()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateTime(String,Time).
    **/
    public void Var163()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateTimestamp(int,Timestamp).
    **/
    public void Var164()
    {
       String key_ = "RowSetListener";
    ...
    /**
    *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed  - updateTimestamp(String,Timestamp).
    **/
    public void Var165()
    {
       String key_ = "RowSetListener";
   ...
   PasswordVault.clearPassword(charPassword);
                ...
                assertCondition((isClosed1 == false) && (isClosed2 == true), "isClosed1="+isClosed1+" isClosed2="+isClosed2+" object is "+rowset.getClass().getName() );
    ...
    /**
     *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed - 
    ...
    public void Var177()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                JDReflectionUtil.callMethod_V(rowset, "updateAsciiStream", 14, stringToAsciiStream("Dublin"), 6L);
    ...
    public void Var178()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                JDReflectionUtil.callMethod_V_IS(rowset, "updateAsciiStream", "C_VARCHAR_50", stringToAsciiStream("Ibiza"), 5L);
    ...
    public void Var180()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                byte[] ba = new byte[] { (byte) -4, (byte) 98, (byte) 99 };
                //rowset.updateBinaryStream ("C_VARBINARY_20", new ByteArrayInputStream (ba), 3);
                JDReflectionUtil.callMethod_V_IS(rowset, "updateBinaryStream", "C_VARBINARY_20", new ByteArrayInputStream(ba), 3L);
    ...
    /**
     * Validates that addRowSetListener(RowSetListener) notifies
     * listeners when the row is changed - updateBlob().
     */
    public void Var181()
    {
        if (checkJdbc40())
    ...
    /**
     * Validates that addRowSetListener(RowSetListener) notifies
     * listeners when the row is changed - updateCharacterStream().
     */
    public void Var184()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                //rowset.updateCharacterStream ("C_VARCHAR_50", new StringReader ("Gillette"), 8);
                JDReflectionUtil.callMethod_V(rowset, "updateCharacterStream", "C_VARCHAR_50", new StringReader("Gillette"), 8L);
    ...
    /**
     * Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed -  updateClob().
     **/
    public void Var185()
    {
        if (checkJdbc40())
    ...
    /**
     * Validates that addRowSetListener(RowSetListener) notifies
     * listeners when the row is changed - updateNCharacterStream().
     */
    public void Var188()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                //rowset.updateNCharacterStream ("C_VARCHAR_50", new StringReader ("Gillette"), 8);
                JDReflectionUtil.callMethod_V(rowset, "updateNCharacterStream", "C_VARCHAR_50", new StringReader("Gillette"), 8L);
    ...
    /**
     * Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed -  updateNClob().
     **/
    public void Var189()
    {
        if (checkJdbc40())
    ...
    /**
     *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed - updateNClob2().
     **/
    public void Var191()
    {
        if (checkJdbc40())
    ...
    /**
     *  Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed -  updateRowId().
     **/
    public void Var195()
    {
        if (checkJdbc40())
    ...
    /**
     * Validates that addRowSetListener(RowSetListener) notifies listeners when the row is changed -  updateSQLXML().
     **/
    public void Var197()
    {
        if (checkJdbc40())
    ...
    public void Var199()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                //rowset.updateAsciiStream (14, stringToAsciiStream ("Dublin"), 6L);
                JDReflectionUtil.callMethod_V(rowset, "updateAsciiStream", 14, stringToAsciiStream("Dublin2"), 7L);
                rowset.updateRow();
                rowset.close();
                ...
                validater.close();
                assertCondition(v.equals("Dublin2"));
                rowset.close();
            } catch (Exception e)
            {
                failed(e, "Unexpected Exception.");
    ...
    public void Var200()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                //rowset.updateAsciiStream (14, stringToAsciiStream ("Dublin"), 6L);
                JDReflectionUtil.callMethod_V_IS(rowset, "updateAsciiStream", "C_VARCHAR_50", stringToAsciiStream("Dublin3"), 7L);
                rowset.updateRow();
                rowset.close();
    ...
    public void Var210()
    {
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                //rowset.updateAsciiStream (14, stringToAsciiStream ("Dublin"), 6L);
                JDReflectionUtil.callMethod_V(rowset, "updateNCharacterStream", "C_VARCHAR_50", new StringReader("Dublin9"), 7L);
                rowset.updateRow();
                rowset.close();
    ...
    public void Var217()
    {
        ...
        if (checkJdbc40())
        {
            ...
            try
            {
                //get any valid rowid from the get table
                rowset2 = getRowSet(jndiName_);
                rowset2.setUsername(systemObject_.getUserId());
                rowset2.setPassword(PasswordVault.decryptPasswordLeak(encryptedPassword_));
    ...
    public void Var218()
    {
        ...
        if (checkJdbc40())
        {
            ...
            try
            {
                ...
                /* Object ri = */  JDReflectionUtil.callMethod_O( rowset, "getRowId", 28);
                //note, that as of today, i cannot update the rowid with new values (error from host server)
    ...
    /**
updateBoolean() - Should work when the column index is valid on a BOOLEAN column 
**/
    public void Var221()
    {
       String key_            = "JDRSUpdateBoolean";
