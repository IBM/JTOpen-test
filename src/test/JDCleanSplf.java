...
public class JDCleanSplf implements PrintObjectListListener{
  ...
  public static void main(String args[]) {
    try {
      ...
      JDCleanSplfResults results = deleteSpoolFiles(System.out, as400, "", startFilterTime, endFilterTime, deleteAll, deleteUserIds); 
	  ...
	  JDCleanSplfResults uselessResults = deleteUselessSpoolFiles(System.out, as400, "", startFilterTime, System.currentTimeMillis());
  ...
  public static JDCleanSplfResults deleteSpoolFiles(PrintStream out, AS400 as400, String nestLevel, long startFilterTime, long endFilterTime, boolean deleteAll, Hashtable deleteUserIds ) throws Exception  {
    ...
    out.println(nestLevel+"Attempting deleteSpoolFiles "+startDateFilter+":"+startTimeFilter+" - "+endDateFilter+":"+endTimeFilter); 
    if (startDateFilter.equals(endDateFilter) &&
	startTimeFilter.equals(endTimeFilter)) {
    ...
    try { 
      ...
      return deleteSpoolFilesAttempt(out, as400, nestLevel, startFilterTime, endFilterTime, deleteAll, deleteUserIds);
    } catch (Exception e) { 
	  ...
	  (message.indexOf("ConnectionDropped") >= 0)) {
	  if (message.indexOf("CPF34C4") >= 0) { 
	  ...
	  out.println(nestLevel+"Error after "+seconds+" seconds for "+startDateFilter+":"+startTimeFilter+" - "+endDateFilter+":"+endTimeFilter); 
	...
	if (quarterTime < 1000) {
        ...
        JDCleanSplfResults firstResults = deleteSpoolFiles(out, as400, nestLevel+"a ", 
            startFilterTime , startFilterTime+quarterTime, 
        ...
        JDCleanSplfResults fourthResults = deleteSpoolFiles(out, as400, nestLevel+"d ", 
	      ...
	      JDCleanSplfResults newResults = new JDCleanSplfResults(firstResults,secondResults,thirdResults,fourthResults);
        ...
        out.println(nestLevel+"DONE         for "+startDateFilter+":"+startTimeFilter+
            ...
            " deleteCount="+newResults.deleteCount+
            " deleteBytes="+numberFormat.format(newResults.deleteBytes)+
            " keepCount="+newResults.keepCount+
            " keepBytes="+numberFormat.format(newResults.keepBytes)+
	    " processCount="+newResults.processedCount+
  ...
  public static JDCleanSplfResults deleteUselessSpoolFiles(PrintStream out, AS400 as400, String nestLevel, long startFilterTime, long endFilterTime ) throws Exception  {
    ...
    String endDateFilter = getDateFilter(endFilterTime); 
    String endTimeFilter = getTimeFilter(endFilterTime); 
    out.println(nestLevel+"Attempting deleteUselessSpoolFile "+startDateFilter+":"+startTimeFilter+" - "+endDateFilter+":"+endTimeFilter); 
	...
	if (quarterTime < 1000) {
	    ...
	    JDCleanSplfResults resultsA = deleteUselessSpoolFiles(out, as400, nestLevel+"a ", 
								  startFilterTime , startFilterTime+quarterTime);
...
public static JDCleanSplfResults deleteSpoolFilesAttempt(PrintStream out, AS400 as400, String nestLevel, long startFilterTime, long endFilterTime, boolean deleteAll, Hashtable deleteUserIds ) throws Exception  {
  ...
  if (verbose) out.println(nestLevel+"Getting list for "+startDateFilter+":"+startTimeFilter+" - "+endDateFilter+":"+endTimeFilter); 
  ...
  while (objects.hasMoreElements()) { 
    ...
    if (o instanceof SpooledFile) {
      ...
      SpooledFile spooledFile = (SpooledFile) o;
      if (deleteAll) { 
        doDelete = true; 
      } else { 
        if (deleteUserIds.get(spooledFile.getJobUser()) != null) { 
          doDelete = true; 
        }
      }
      if (doDelete) { 
      System.out.println(nestLevel+"Deleting "+  
            spooledFile.getCreateDate() + " " + 
                          spooledFile.getCreateTime() + " " +
                          spooledFile.getName() + " " +
      ...
      spooledFile.delete();
        ...
        System.out.println(nestLevel+"Keeping  "+  
            spooledFile.getCreateDate() + " " + 
                          spooledFile.getCreateTime() + " " +
                          spooledFile.getName() + " " +
                          spooledFile.getNumber() + " " + 
      ...
      System.out.println(nestLevel+"Object is "+o);
    }
  ...
  */ 
  ...
  // Make sure the default timeout is at least twice as large as the time it takes to 
  ...
  out.println(nestLevel+"DONE         for "+startDateFilter+":"+startTimeFilter+" - "+endDateFilter+":"+endTimeFilter+
     " deleteCount="+deleteCount+
     " deleteBytes="+numberFormat.format(deleteBytes) + 
     " keepCount="+keepCount+
     " keepBytes="+numberFormat.format(keepBytes) + 
		     " processCount="+processCount+" time="+hours+":"+minutes+":"+seconds);
  return new JDCleanSplfResults(deleteCount, deleteBytes, keepCount, keepBytes, processCount, processSeconds); 
}



public static JDCleanSplfResults deleteUselessSpoolFilesAttempt(PrintStream out, AS400 as400, String nestLevel, long startFilterTime, long endFilterTime) throws Exception  {
  SpooledFileList spooledFileList  = new SpooledFileList(as400);
  spooledFileList.setUserFilter("*ALL");
  ...
  while (objects.hasMoreElements()) { 
    ...
    if (o instanceof SpooledFile) {
      ...
      if (doDelete) { 
      out.println(nestLevel+"Deleting "+  
            spooledFile.getCreateDate() + " " + 
                          spooledFile.getCreateTime() + " " +
                          spooledFile.getName() + " " +
                          spooledFile.getNumber() + " " + 
      ...
      out.println(nestLevel+"Object is "+o);
    }
...
static void waitForComplete(PrintStream out, AS400 as400, String nestLevel, SpooledFileList spooledFileList, JDCleanSplf listener, long endFilterTime  ) throws Exception {
...
public void listObjectAdded(PrintObjectListEvent e) {
  ...
  if (processCount_ % 1000 == 0) {
      out.println(nestLevel_+"  processCount="+processCount_+" deleteCount="+deleteCount_+" keepCount="+keepCount_);
  ...
  if (printObject instanceof SpooledFile) {
    ...
    if (deleteAll_) { 
      ...
      if (jobUser == null) { 
         out.println("Error:  jobUser is null"); 
          ...
          try {
            int pageCount = spooledFile.getSingleIntegerAttribute(
                PrintObject.ATTR_PAGES).intValue();
            ...
            if (pageCount <= 100) {
              ...
              String data = charConverter_.byteArrayToString(buffer, 0,
                  dataLength);
    ...
    if (doDelete) { 
      if (verbose) { out.println(nestLevel_+ "Deleting "+  
          spooledFile.getCreateDate() + " " + 
                        spooledFile.getCreateTime() + " " +
                        spooledFile.getName() + " " +
                        spooledFile.getNumber() + " " + 
	...
	keepCount_++;
      ...
      if (verbose) { 
	  out.println(nestLevel_+ "Keeping  "+  
			     spooledFile.getCreateDate() + " " + 
			     spooledFile.getCreateTime() + 
			     " FILE(" + spooledFile.getName() + ")" +
			     " SPLNBR("+spooledFile.getNumber() + ") " + 
			     " JOB("+spooledFile.getJobNumber()+
			     "/"+spooledFile.getJobUser() + 
...
private boolean hasQprintNoise(String data) {
  String[] qprintNoise = {
     ...
     "added manifest",
     " A normal program ",
     " loopdead loopdead loopdead ",
     " start to sleep 10s done",
     " Internal spool control file not accessible.",
     "THE SQL COMMAND COMPLETED SUCCESSFULLY",
     "Starting Deployment",
     "Starting JobQueue",
     "submitted to job queue",
     "Error found creating directory",
     " Error found opening file",
     "Output queue changed to QPRINT",
     "CPF2110:  Library",
     "CPF7020:  Journal receivers",
...
static String [][] prefixesAndNoise = {
    { "MCH", },
    { "SQL",
      ...
      "SQL0950", /* database not in RDBDIRE"
      /* SQL0952 processing of statement ended */
      "SQL5016", /* qualified object name not valid */ 
      "SQL7022", /* user not same as current user */ 
      "SQL7908",
      "SQL792A", /* schema not created */
    ...
    { "CPF",
       ...
       "CPF3485",
       "CPF338C", /* Internal spool control file not accessible. */
    ...
    { "CPC",
      "CPC0905", /*Subsystem &3 prestart job entry not active */
...
/* "CPC1166", time limit reached for SIGTERM handler */ 
   ...
   /* "CPC1224" Job Ended abnormally */
      ...
      "CPCA980",
      "CPCA981",
      "CPCA984", /* trace option changed" */ 
      "CPCA986", /* user trace dumped */
      },
    { "CPI",
      "CPI0952", /* Start of prestart jobs in progress */
    ...
    { "JVA", 
        "JVAB302", 
        "JVAB578", /* JVM properties loaded from file */
        },
    { "QSH",
      "QSH0005", /*  Command ended normally */
    },
    { "*NO",
      "*NONE",
    },
           ...
           if (message != null) {
             ...
             if (!foundMessage) { 
               ...
               if (verbose) {
                  ...
                  if (lastOffset > data.length()) lastOffset=data.length();
...
private String extractMessage(String data, int searchOffset) {
  ...
  if ((data.charAt(searchOffset+7) == ' ') &&  
      isMessageDigit(data.charAt(searchOffset+3)) && 
      isMessageDigit(data.charAt(searchOffset+4)) && 
      isMessageDigit(data.charAt(searchOffset+5)) && 
...
public JDCleanSplfResults(long deleteCount, long deleteBytes, long keepCount, long keepBytes, long processedCount, long processSeconds) {
