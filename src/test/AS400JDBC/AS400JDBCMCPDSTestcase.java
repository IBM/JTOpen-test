...
public class AS400JDBCMCPDSTestcase extends Testcase {
   ...
   **/
  public AS400JDBCMCPDSTestcase(AS400 systemObject, Vector<String> variationsToRun, int runMode,
      FileOutputStream fileOutputStream,

      String password, String jndiType, String ldapUsr, String ldapPwd, String systemName)

  {
    super(systemObject, "AS400JDBCMCPDSTestcase", variationsToRun, runMode, fileOutputStream, password);
   ...
   * 
   * AS400JDBCManagedConnectionPoolDataSource() setServerName() setUser()
   * setPassword() getConnection()
   **/
  @SuppressWarnings("deprecation")
  public void Var001() {
    Connection c = null;
    AS400JDBCManagedConnectionPoolDataSource ds = null;
    try {
      ds = new AS400JDBCManagedConnectionPoolDataSource();
  ...
  public void Var002() {
    ...
    try {
      char[] charPassword = PasswordVault.decryptPassword(encryptedPassword_);
      ds = new AS400JDBCManagedConnectionPoolDataSource(systemObject_.getSystemName(), systemObject_.getUserId(),
          charPassword);
      PasswordVault.clearPassword(charPassword);
      c = ds.getConnection();
  ...
  /**
   * Validates that getConnection(String,String) returns a valid connection
   * object.
   * 
   * AS400JDBCManagedConnectionPoolDataSource() setServerName()
   ...
   * 
   * AS400JDBCManagedConnectionPoolDataSource(String,String,String)
   * setServerName() getConnection(String,String)
   **/
  @SuppressWarnings("deprecation")
  public void Var004() {
    Connection c = null;
  ...
  /**
   * Validates that a second getConnection(String,String) returns a valid
   * connection object.
   * 
   * AS400JDBCManagedConnectionPoolDataSource() setServerName()
   * getConnection(String,String)
  ...
  public void Var008() {
    ...
    try {
      ...
      ResultSet rs = s1.executeQuery("VALUES CURRENT USER");
      String currentUser = "NOTSET";
      if (rs.next()) {
        currentUser = rs.getString(1);
      }
      rs.close();
      s1.close();
      if (c != null && c2 != null && currentUser.equalsIgnoreCase(userId_))
        succeeded();
      else
        failed("Unexpected results. c=" + c + " c2=" + c2 + " sql.currentUser=" + currentUser + " connect.userId_="
            + userId_);
    } catch (Exception e) {
   ...
   * 
   * AS400JDBCManagedConnectionPoolDataSource() setUser() setPassword()
   * getConnection()
   **/
  @SuppressWarnings("deprecation")
   ...
   * 
   * AS400JDBCManagedConnectionPoolDataSource() setServerName() setPassword()
   * getConnection()
   **/
  @SuppressWarnings("deprecation")
  ...
  public void Var014() {
    ...
    try {
      char[] charPassword = PasswordVault.decryptPassword(encryptedPassword_);
      AS400JDBCManagedConnectionPoolDataSource ds = new AS400JDBCManagedConnectionPoolDataSource(
          systemObject_.getSystemName(), systemObject_.getUserId(), charPassword);
      PasswordVault.clearPassword(charPassword);
  ...
  /**
   * Validates that getPooledConnection(String,String) returns a valid pooled
   * connection.
   **/
  @SuppressWarnings("deprecation")
  public void Var016() {
    ...
    try {
      ...
      char[] charPassword = PasswordVault.decryptPassword(encryptedPassword_);
      AS400JDBCPooledConnection pc = (AS400JDBCPooledConnection) ds.getPooledConnection(systemObject_.getUserId(),
          charPassword);
      PasswordVault.clearPassword(charPassword);
  ...
  public void Var017() {
    ...
    try {
      ...
      if ((ds2.getServerName().equals("") || ds2.getServerName().equals("localhost")) && ds2.getUser().equals(userName)
          && ds2.getDatabaseName().equals("") && ds2.getDataSourceName().equals("") && ds2.getDescription().equals(""))
        succeeded();
      else {
        output_.println(ds2.getServerName() + "; " + ds2.getUser() + "; " + ds2.getDatabaseName() + "; "
            + ds2.getDataSourceName() + "; " + ds2.getDescription());
        failed("Unexpected results.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
  ...
  public void Var018() {
    ...
    try {
      ...
      if (ds2.getServerName().equalsIgnoreCase(serverName) && ds2.getUser().equals(userName)
          && ds2.getDatabaseName().equals("") && ds2.getDataSourceName().equals("") && ds2.getDescription().equals(""))
        succeeded();
      else
        failed("Unexpected results.");
  ...
  public void Var019() {
    ...
    try {
      ...
      if ((ds2.getServerName().equals("") || ds2.getServerName().equals("localhost"))
          && ds2.getUser().equalsIgnoreCase(user) && ds2.getDatabaseName().equals("")
          && ds2.getDataSourceName().equals("") && ds2.getDescription().equals(""))
        succeeded();
      else
        failed("Unexpected results.");
    } catch (Exception e) {
  ...
  /**
   * Validates that serialization works as expected with the databaseName,
   * dataSourceName, and description set.
   **/
  @SuppressWarnings("deprecation")
  public void Var020() {
    ...
    try {
      ...
      if ((ds2.getServerName().equals("") || ds2.getServerName().equals("localhost")) && ds2.getUser().equals(userName)
          && ds2.getDatabaseName().equals(databaseName) && ds2.getDataSourceName().equals(dataSourceName)
  ...
  public void Var021() {
    ...
    try {
      ...
      if ((ds2.getServerName().equals("") || ds2.getServerName().equals("localhost")) && ds2.getUser().equals(userName)
          && ds2.getDatabaseName().equals("") && ds2.getDataSourceName().equals("") && ds2.getDescription().equals("")
          && ds2.getDateFormat().equals(dateFormat) && ds2.isPackageCache())
        succeeded();
      else
        failed("Unexpected results.");
    } catch (Exception e) {
  ...
  /**
   * Validates that a datasource object can be set into JNDI, and then
   * instantiated from JNDI.
   **/
  @SuppressWarnings("deprecation")
  public void Var022() {
    if (environment_ == OS_AS400) {
    ...
    AS400JDBCManagedConnectionPoolDataSource dataSource = null;
    AS400JDBCManagedConnectionPoolDataSource jndiDataSource = null;
    try {
      // Create a data source to the AS/400 database.
      dataSource = new AS400JDBCManagedConnectionPoolDataSource();
      ...
      jndiDataSource = (AS400JDBCManagedConnectionPoolDataSource) context_.lookup(jndiName);
      char[] charPassword = PasswordVault.decryptPassword(encryptedPassword_);
      ...
      if (rsNext && jndiDataSource.getServerName().equals(serverName) &&
      ...
      if (jndiDataSource != null)
        jndiDataSource.closePool();
    }
  }

  /**
   * Validates that a default datasource object can be set into JNDI, and then
   * instantiated from JNDI.
   **/
  @SuppressWarnings("deprecation")
  public void Var023() {
  ...
  /**
   * Validates that a datasource object with serverName and user can be created
   * set into JNDI, and then instantiated from JNDI.
   **/
  @SuppressWarnings("deprecation")
  public void Var024() {
    if (environment_ == OS_AS400) {
    ...
    try {
      ...
      s.close();
      connection.close();
      if (rsNext && jndiDataSource.getServerName().equals(systemObject_.getSystemName())
  ...
  /**
   * Validates that a datasource object with serverName and user can be set into
   * JNDI, and then instantiated from JNDI and finally used to get a connection
   * after changing properies.
   **/
  @SuppressWarnings("deprecation")
  public void Var025() {
    if (environment_ == OS_AS400) {
  ...
  public void Var026() {
    ...
    try {
      ...
      s.close();
      connection.close();
      if (rsNext && jndiDataSource.getDateFormat().equals(dateFormat) && jndiDataSource.isExtendedDynamic())
        succeeded();
      else
        failed("Unexpected results.");
  ...
  public void Var028() {
    ...
    try {
      char[] charPassword = PasswordVault.decryptPassword(encryptedPassword_);
      ds = new AS400JDBCManagedConnectionPoolDataSource(systemObject_.getSystemName(), userId_, charPassword);
      PasswordVault.clearPassword(charPassword);
      ds.setLogWriter(writer_);
      ds.setPrompt(false);
  ...
  /**
   * Validates that getConnection().getMetaData() returns a valid database meta
   * data object.
   * 
   * AS400JDBCManagedConnectionPoolDataSource(String,String,String)
   * getConnection()
  ...
  @SuppressWarnings("deprecation")
  public void Var030() {
    AS400JDBCManagedConnectionPoolDataSource cpds = null;
    try {

      char[] charPassword = PasswordVault.decryptPassword(encryptedPassword_);
      cpds = new AS400JDBCManagedConnectionPoolDataSource(systemObject_.getSystemName(), systemObject_.getUserId(),
          charPassword);
      ...
      // ok, if running on i5, and using the hostname of something other than
      // localhost, but still running
      // to the local i5, then you still get back "localhost", so use systemname from
      // as400 object
   ...
   **/
  private AS400JDBCManagedConnectionPoolDataSource serialize(AS400JDBCManagedConnectionPoolDataSource ds)
      throws Exception {
    // Serialize.
