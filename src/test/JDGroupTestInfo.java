...
public class JDGroupTestInfo {
  ...
  /**
   * List of subsystem used by group test.  If there are more than
   * a certain threshold of number of jobs running in these subsystems, 
   * then do not start 
   * additional tests. 
   */
  public static String[] groupTestSubsystems = {
    "ASQBATS1", 
    "ASQBATS2", 
    "ASQBATS3", 
    "ASQBATS99", 
  ...
  /**
   * Returns the number of job running group tests for the 
   * specified JDBC server connection.  A server connection is
   * used so that a client can detect if the server is busy
   * running group test. 
   * @param c
   * @return
   */
  public static int getRunningGroupTestJobCount(Connection c, StringBuffer sb ) throws SQLException  {
    ...
    /*                            12345678911234567892123456789312345678941234567895 */
	  try { 
	      s.execute("CALL QSYS.QCMDEXC('CRTSRCPF FILE(QGPL/"+outputFile+") RCDLEN(162)          ',000000050.00000)");
	  } catch (Exception e) {
    ...
    // Run wrkactjob and place the results in dbfile
     ...
     /*  12345678911234567892123456789312345678941234567895123456789612345678971234567898123456789912345678901234567891123456789112345678921234567893123456 78941234567895 */              
	    "QSH CMD(''system wrkactjob > /qsys.lib/qgpl.lib/"+outputFile+".file/"+outputFile+".mbr'')                                      '," +
	    "000000100.00000)";
