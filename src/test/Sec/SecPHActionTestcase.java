 ...
 * <p>
 * Test variations for the following actions:
 * <ul>
 * <li>refresh
 ...
 * </ul>
 * <p>
 * Test variations for the following related events:
 * <ul>
 * <li>created
 * <li>refreshed
 * <li>swapped
...
public class SecPHActionTestcase extends Testcase implements AS400CredentialListener {
  ...
  public void Var002() {
    try {
      ...
      try {
        ...
        assertCondition(pt.getAutomaticRefreshStatus() == AS400Credential.CR_AUTO_REFRESH_NOT_VALID && ace_ == null
            && refreshCount_ == 0, "Unexpected refresh event signalled.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
    }
  ...
  /**
   * Test successful swap if running on the local host; otherwise verify the
   * exception. Verify no return credential is returned even if requested.
   **/
  public void Var003() {
    try {
      // Create objects.
      ...
      try {
        ...
        String idAgain = as400_2.getUserId();
        as400_2.close();
        assertCondition(
            originalID.equals(idAgain) && ace_ != null && ace_.getID() == AS400CredentialEvent.CR_SWAP && cr == null,
            "Swap failed.");
      } catch (SwapFailedException sfe) {
        assertCondition((!isNative_ || !profileHandleImplNativeAvailable)
            && sfe.getReturnCode() == AS400SecurityException.REQUEST_NOT_SUPPORTED, "Swap failed.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
    }
  ...
  public void Var004() {
    try {
      ...
      try {
        ...
        pt.swap();
        AS400 as400_2 = new AS400();
        String userIdAfterSwap = as400_2.getUserId();
        if (SecAuthTest.uid2.equals(userIdAfterSwap)) {
          // Swap back.
          ph.swap();
          AS400 as400_3 = new AS400();
          String userIdAfterSwapBack = as400_3.getUserId();

          as400.close();
          as400_2.close();
          as400_3.close();
          assertCondition(
              (originalID.equals(userIdAfterSwapBack) && ace_ != null && ace_.getID() == AS400CredentialEvent.CR_SWAP)
                  || !isNative_ || !profileHandleImplNativeAvailable,
              "Unexpected result.");
        } else {
          as400_2.close();
          as400.close();
  ...
  public void Var005() {
    try {
      ...
      try {
        ...
        as400.close();
        assertCondition(ise.getReturnCode() == ExtendedIllegalStateException.PROPERTY_NOT_SET,
            "Expected return code not found.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
    }
  ...
  /**
   * Test failed swap (generic/no parms) for a credential that was previously
   * destroyed.
   **/
  public void Var006() {
    try {
      ...
      try {
        ...
        String idAfterError = as4002.getUserId();
        as4002.close();
        assertCondition(
            (!isNative_ || !profileHandleImplNativeAvailable || originalID.equals(idAfterError)) && ace_ == null
                && ise.getReturnCode() == ExtendedIllegalStateException.PROPERTY_NOT_SET,
            "Unexpected exception during swap.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
    }
  ...
  /**
   * Test successful swap if running on the local host; otherwise not applicable.
  ...
  public void Var007() {
    try {
      ...
      String originalID = sys2.getUserId();
      sys2.close();
      ph.setSystem(sys);
      ph.addCredentialListener(this);
      ph.setHandle();
      ...
      try {
        // Swap.
        pt.swap(false);
        AS400 sysAfterSwap = new AS400();
        String useridAfterSwap = sysAfterSwap.getUserId();
        if (SecAuthTest.uid2.equals(useridAfterSwap)) {
          // Swap back.
          AS400Credential cr = ph.swap(false);
          AS400 sysAfterSwapBack = new AS400();
          String useridAfterSwapBack = sysAfterSwapBack.getUserId();
          sysAfterSwapBack.close();
          sysAfterSwap.close();
          assertCondition(originalID.equals(useridAfterSwapBack) && ace_ != null
              && ace_.getID() == AS400CredentialEvent.CR_SWAP && cr == null, "Unexpected result.");
        } else {
          sysAfterSwap.close();
  ...
  /**
   * Test successful destroy of an existing credential if running on the local
   * host; otherwise verify the exception.
   **/
  public void Var008() {
    try {
      // Create objects.
      ...
      try {
        ...
        if ((isNative_ && profileHandleImplNativeAvailable)
            || rfe.getReturnCode() != AS400SecurityException.REQUEST_NOT_SUPPORTED) {
          throw rfe;
        }
      }
      // Reset internal state.
      ...
      try {
        // Perform test.
        ph.destroy();
        assertCondition(ph.isDestroyed() && ace_ != null && ace_.getID() == AS400CredentialEvent.CR_DESTROY,
            "Unexpected failure.");
      } catch (DestroyFailedException dfe) {
        assertCondition((!isNative_ || !profileHandleImplNativeAvailable)
            && dfe.getReturnCode() == AS400SecurityException.REQUEST_NOT_SUPPORTED, "Destroy failed.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
    }
  ...
  /**
   * Test successful create & corresponding event if running on the local host;
   * otherwise verify the exception.
   **/
  public void Var010() {
    try {
      // Reset internal state.
      ...
      try {
        // Perform test.
        ph.setHandle();
        assertCondition(ace_ != null && ace_.getID() == AS400CredentialEvent.CR_CREATE,
            "Expected event not signalled.");
      } catch (RetrieveFailedException rfe) {
        assertCondition((!isNative_ || !profileHandleImplNativeAvailable)
            && rfe.getReturnCode() == AS400SecurityException.REQUEST_NOT_SUPPORTED, "Create failed.");
      }
    } catch (Exception e) {
      failed(e, "Unexpected exception.");
    }
  ...
  /**
   * Test unsuccessful destroy of a previously destroyed credential if running on
   * the local host; otherwise verify the exception.
   **/
  public void Var017() {
    try {
      // Create objects.
