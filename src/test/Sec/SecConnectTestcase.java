...
public class SecConnectTestcase extends Testcase
{
   ...
   PasswordVault.clearPassword(charPassword);
    ...
    private final boolean testConnection(AS400 sys, int service)
    {
      ...
      {
        try { sys.disconnectAllServices(); } catch (Throwable t) { t.printStackTrace(); }
      }
      return succeeded;
    }
    ...
    private final boolean testConnectionToService(AS400 sys, int service)
    {
      ...
      try
      {
        ...
        //System.out.println("isConnectionAlive("+nameForService(service)+") returned " + result);
        if (result == true) {
        ...
        /* If attended:
        if (service == AS400.DATABASE)
        {
          System.out.println ("Signon to the system and end the job QUSRWRK/QZDASOINIT under user \"" + userId_ + "\".  Press ENTER to continue."); try { System.in.read (); } catch (Exception exc) {};

          System.out.println("calling isConnectionAlive ...");
          result = sys.isConnectionAlive();
          System.out.println("isConnectionAlive() returned " + result);
          if (result == true) {
            System.out.println("UNEXPECTED RESULT (X)");
            succeeded = false;
          }
        }
        */
