...
public class SecChgPwdDialogTestcase extends Testcase {
  public static void main(String args[]) throws Exception {
    ...
    newArgs[0] = "-tc";
    newArgs[1] = "SecChgPwdDialogTestcase";
   ...
   * and test the minimize, maximize, focus, etc. Then cancel before changing the
   * expired password.
   **/
  public void Var001() {
    if (runMode_ != UNATTENDED) {
      try {
        ...
        setupUserID();
        try {
          AS400 sys = new AS400(systemName_, "JAVAEXPIR", "MY1PWD".toCharArray());
  ...
  /**
   * Create a user with an expired password, invoke the change password dialog.
   * Verify: - an error is displayed if old password is incorrect - an error is
   * displayed if confirm password wasn't input by user - an error is displayed if
   * confirm and new password don't match
   **/
  public void Var002() {
    if (runMode_ != UNATTENDED) {
      try {
        ...
        try {
          ...
          if (JTOpenTestEnvironment.isOS400 == false) {
            ...
            output_.println(
                "7) Verify 'New password and confirm password are not the same.' message comes up; select OK.");
            ...
            output_.println(
                "10) Verify 'New password and confirm password are not the same.' message comes up; select OK.");
  ...
  /**
   * Create a user with an expired password, invoke the change password dialog.
   * Verify: - password is changed successfully when old, new and confirm fields
   * are input correctly
   **/
  public void Var003() {
    if (runMode_ != UNATTENDED) {
  ...
  public void Var004() {
    try {
      ...
      try {
        ...
        AS400 sys = new AS400(systemName_, "javaexpir", "MY1PWD".toCharArray());
        sys.setMustUseSockets(mustUseSockets_);
        String newPassword = "JTEAM1JVJTEAM1JVJTEAM1JVJTEAM1JVJTEAM1JVJTEAM1JVJTEAM1JV".substring(0,
            minlenInteger.intValue());
        sys.changePassword("MY1PWD".toCharArray(), newPassword.toCharArray());
        if (sys.validateSignon(newPassword.toCharArray())) {
          sys.close();
          succeeded();
        } else {
          sys.close();
