...
public class SecPortMapperTestcase extends Testcase
{
   ...
   PasswordVault.clearPassword(charPassword);
            ...
            int port = system.getServicePort(service);
            assertCondition(port == AS400.USE_PORT_MAPPER, "Port incorrect: " + port+ " sb USE_PORT_MAPPER="+AS400.USE_PORT_MAPPER);
        }
        catch (Exception e)
        {
            failed(e, "Unexpected exception.");
   ...
   PasswordVault.clearPassword(charPassword);
            ...
            try
            {
                int port = system.getServicePort(service);
                String failMessage = "";
                if (port != expectedPort) failMessage += "Port incorrect: " + port + " sb "+expectedPort+"\n";
