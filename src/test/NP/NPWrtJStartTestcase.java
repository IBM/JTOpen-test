  ...
  public static void main(String args[]) throws Exception {
	...
	boolean allVariations = (variationsToRun_.size() == 0);
        ...
        try
            ...
            if (cmd.run("QSYS/CRTOUTQ OUTQ(NPJAVA/STARTTST) AUTCHK(*DTAAUT)") == false)
                {
            ...
            if (cmd.run("QSYS/GRTOBJAUT OBJ(NPJAVA/STARTTST) OBJTYPE(*OUTQ) USER(*PUBLIC)") == false)
                {
...
$$$ TO DO $$$ - delete this line */
            ...
            if (cmd.run("QSYS/CLROUTQ OUTQ(NPJAVA/STARTTST)") == false)
                {
            ...
            if (cmd.run("QSYS/DLTOUTQ OUTQ(NPJAVA/STARTTST)") == false)
                {
    ...
    public void Var008()
    {
        try
            ...
            CommandCall cmd = new CommandCall(systemObject_);
            if (cmd.run("QSYS/CRTOUTQ OUTQ(NPJAVA/JAVAOUTQ) AUTCHK(*DTAAUT)") == false)
                {
                failed("Could not create an output queue. "
                       + cmd.getMessageList()[0].getID()
                       + ": " + cmd.getMessageList()[0].getText());
            ...
            if (cmd.run("QSYS/GRTOBJAUT OBJ(NPJAVA/JAVAOUTQ) OBJTYPE(*OUTQ) USER(*PUBLIC)") == false)
                {
                failed("Could not grant *public authority to output queue. "
                       + cmd.getMessageList()[0].getID()
                       + ": " + cmd.getMessageList()[0].getText());
                ...
                // clear the output queue we created.
                if (cmd.run("QSYS/CLROUTQ OUTQ(NPJAVA/JAVAOUTQ)") == false)
                    {
                    output_.println("Could not clear output queue we created. "
                                    + cmd.getMessageList()[0].getID()
                                    + ": " + cmd.getMessageList()[0].getText());
                ...
                // delete the output queue we created.
                if (cmd.run("QSYS/DLTOUTQ OUTQ(NPJAVA/JAVAOUTQ)") == false)
                    {
                    output_.println("Could not delete output queue we created. "
                                    + cmd.getMessageList()[0].getID()
                                    + ": " + cmd.getMessageList()[0].getText());
