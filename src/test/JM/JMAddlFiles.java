   ...
   public void Var003 ()
    {
      try {
        ...
        File entry2 = new File ("entry2");
        File entry3 = new File ("entry3");
        Vector<File> inList = new Vector<File> ();
        inList.add (entry1);
        inList.add (entry2);
        inList.add (entry3);
 ...
 **/
    ...
    public void Var009 ()
    {
      JarMaker jm = new JarMaker ();
      try {
        Vector<Comparable<?>> inList = new Vector<Comparable<?>> (0);
        jm.setAdditionalFiles (inList);
        Vector<?> outList = jm.getAdditionalFiles ();
        if (inList.size () != 0)
          failed ("Reported files list is not zero-length."+outList);
        else
        {
 ...
 **/
    ...
    public void Var012 ()
    {
      ...
      try {
        ...
        if (!JMTest.compareFileLists (outList, expected1))
          failed ("Lists don't match.");
        else
        {
          // Remove the destination jar file if it exists.
 ...
 **/
    ...
    public void Var017 ()
    {
      ...
      try {
        ...
        Vector<File> inList2 = new Vector<File> (1);
        inList2.add (JMTest.ADDITIONAL_FILE_3);
