...
public class CleanupJavaTestJobs {
  public static void usage() {
    System.out.println("Usage:  java CleanupJavaTestJobs [<SYSTEM> <USERID> <PASSWORD> ]");
    System.out.println("   Ends all the QJVAEXEC and CLIJOBURN jobs that are used by testcases and older than 1 hour");
  }
        ...
        for (int i = 0; i < 2; i++) {
          ...
          while (enumeration.hasMoreElements() && continueLoop) {
            ...
            try {
              ...
              // "+j.getNumber()+"/"+j.getUser()+"/"+j.getName()+" user is "+user+" function
              // name is "+functionName);
              if (user.equalsIgnoreCase("JAVA") || user.equalsIgnoreCase("NEWTONJUDF")
                  || user.equalsIgnoreCase("SQLJTEST") || user.equalsIgnoreCase("JDPWRSYS")) {
                System.out.println("-------------------");
                System.out.println("Job name is " + j.getNumber() + "/" + j.getUser() + "/" + j.getName() + " user is "
                    + user + " function name is " + functionName);
                ...
                while (!connectingJobMessageFound && messageEnumeration.hasMoreElements()) {
                  ...
                  if (message.getID().equals("CPF9898")) {
                    ...
                    if ((text.indexOf("Running test") >= 0) || (text.indexOf("starting worker") >= 0)) {
                    ...
                    if (currentTimeMillis - millis > 3600000) {
                      System.out.println(
                          i + ": JVAB302 Message send over an hour ago " + simpleDateFormat.format(date.getTime()));
                      if (i == 0) {
                      ...
                      System.out.println(i + ": JVAB302 Recent message " + simpleDateFormat.format(date.getTime()));
                    }
          ...
          if (line.length() > 0) {
            if (line.indexOf("Caption=") == 0) {
              ...
              if (savedCaption.indexOf("java.exe") >= 0) {
                System.out
                    .println(">" + processId + "," + savedCaption + "," + savedCreationDate + "," + savedCommandLine);
